...
- [x] 後退距離、時間、attemptにhard upper limitがある。
...
- [ ] make autoware-buildと対象package testが成功する（buildとRecovery 43 testは成功、既存trajectory fixture 1件が失敗）。
...
- [x] design.mdを作成する。
- [x] tasklist.mdを作成する。
- [ ] 利用者レビューで対象範囲、段階導入、SIM限定方針を確定する。
...
- [ ] SIM予選での自律REVERSE復帰可否を公式チャットへ確認する。
- [ ] 最大距離、速度、時間、回数、妨害判定を公式チャットへ確認する。
- [ ] online SIMでgear_statusが配信されることを確認する。
- [ ] REVERSE時のacceleration符号を確認する。
- [ ] DRIVE -> REVERSE -> DRIVEのgear report遷移時間を計測する。
- [ ] GearCommandの再送要否と適切なQoSを確認する。
- [ ] reverse時のVelocityReport / odometry signed velocityを確認する。
...
- [ ] baseline実験結果をdesign.mdまたはtasklist末尾へ記録する。
...
- [x] 現行MPC core testの回帰を確認する（Recovery core 26件と既存core testは成功）。
...
- [ ] GearActuationAdapterへ実測済みのreverse加速度意味を実装する。
...
- [x] reverse acceleration、steering、距離、時間にhard limitを追加する。
...
- [ ] gear report確認前に車両が動かないことを確認する。
- [ ] REVERSE中の加速度とsigned speedを記録する。
...
- 公式ルールでは自律後退復帰が明文化されていないためTBDとした。
...
- `test_stuck_recovery_core`は26 / 26、`test_recovery_footprint`は17 / 17成功した。
...
- 未較正値のままactuationを一時的に有効化すると、drive / stop符号、実測停止減速度、
  V2X self-filter契約不足として起動拒否されることを確認した。最終configは全Domain falseへ戻した。
- hostに`pre-commit`がなく、repository固有`.clang-format`もないためformatter / lintは未完了。
  Docker内C++ buildはwarning option付きで成功し、`git diff --check`は成功した。
- package全体testはRecovery新規testが全て成功し、既存
  `final_ver3/traj_mincurv.csv`の重複終端fixture期待1件だけ失敗した。
- AWSIM、REVERSE加速度符号、40 Hz deadline、dev2 / dev3は未検証である。
- 実走確認までは`reverse_actuation_enabled: false`、
  `reverse_acceleration_sign: 0.0`を変更しない。

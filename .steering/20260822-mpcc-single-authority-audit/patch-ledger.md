...
| Overtake shadow/production split / async canonical producer plus `convert_extended_solution_to_legacy()` | current Slice 5 migration | Prove exact canonical artifacts without changing race output | The async producer is now valid, but current-world proof rejected 34/158 eligible cycles; synchronous conversion/circuit/reentry/three-state paths still own production and hide canonical command identity | I-02, I-04, I-05, I-13 | Producer repaired by `output/20260824-043223`; proof coverage gap remains | Close course-frame/corridor proof coverage, then promotion atomically deletes conversion and formulation fallback |

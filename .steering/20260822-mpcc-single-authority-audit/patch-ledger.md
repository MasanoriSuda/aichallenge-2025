...
| Overtake shadow/production split / async canonical producer plus `convert_extended_solution_to_legacy()` | current Slice 5 migration | Prove exact canonical artifacts without changing race output | The async producer is valid and the shared lower course-frame gap is repaired, but Mission transitions still create progress/corridor proof failures; synchronous conversion/circuit/reentry/three-state paths still own production and hide canonical command identity | I-02, I-04, I-05, I-13 | Producer and course-frame provenance repaired by `output/20260824-043223` and `output/20260824-045351`; transition proof gap remains | Explain and repair transition-local progress/corridor provenance, then promotion atomically deletes conversion and formulation fallback |

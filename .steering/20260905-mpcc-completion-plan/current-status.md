...
最新の本番修正は、物理座標と入力連成座標の二つの予測範囲を共通化したf2a89c14。
そのsingle-r10では通常送信551回、1142callback最大20.40ms、期限外の通常送信ゼロ。
Ready中999では物理証明に合格後、実際の送信直前時計が期限を超え、指令を拒否した。
後続の通常送信は再開したが、1016の計画不在・1020の新計画の壁拒否もあり、Start/周回なし。
ユーザーJSON・元DLLは復元/保持し、runtime停止。

999の現在検証は実走16.2ms。R633/R634で同一入力の期限拒否と、新座標側の微分再計算を
分離した。元の親領域に問い合わせ全体が含まれる場合だけ、事前計算を座標変換して利用。
中心点は区間として保持し、元領域を外れる問い合わせ・未対応の座標は再利用しない。
R638既存/新規216件、build126全26/package113全2622/source112が合格。
本番の999再生は11.726–11.835ms、全物理証明と元の実送信期限拒否を保持。
R639保存1986の完全停止・768000項目の包含、R641/R642過去5拒否も合格。
R643で1020の壁拒否は残ると確認。次はこの計算修正のローカルcommitとsingle-r11。
物理モデル・入力集合・壁余裕・期限・唯一の認証済み権限を維持する。
観測/接地モデル、再発進・Start/周回、全M4–M6は未完。
[計算時間と座標変換の設計・証拠](../20260910-mpcc-empirical-plant/receiver-input-enclosure/compiled-coordinate-transport-design.md)。
[二つの予測範囲の設計・証拠](../20260910-mpcc-empirical-plant/receiver-input-enclosure/velocity-correlation-design.md)。

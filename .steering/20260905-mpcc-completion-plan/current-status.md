...
最新検証: 従来座標と入力連成座標の両方で全入力を予測し、対応する範囲の共通部分を
一つの物理検証へ渡す。送信前の短い予測と完全停止までの予測を同じ実装へ統一。
R626既存215件、build125全26、package112全2621、source/architecture112合格。
保存source1986の本番数値計算で完全な壁・停止検証を通過し、256軌道/768000項目は
範囲内。予測単体14.05ms。過去5件の壁・送信期限・停止期限の拒否と元入力の一致を保持。
途中のpackage111走行中Stop4失敗とR625予測不一致2失敗も記録し、条件を維持して修正した。
物理モデル、入力集合、受信・送信窓、壁・停止条件は維持。次はローカルcommitとsingle-r10。
観測/接地モデルの誤差、再発進、Start/周回、全M4–M6は引き続き未完。
[数値表現の設計と証拠](../20260910-mpcc-empirical-plant/receiver-input-enclosure/velocity-correlation-design.md)。

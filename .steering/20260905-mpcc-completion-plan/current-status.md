...
**全体未完。コース収録範囲を0bd08706に修正済み。標準r75は未達。初期通路幅の修正は全build/testを通過し、次は標準r76。**

R472はr75 D1source747/decision1237の同じQP解で通常artifact生成を再現。
旧初期幅は拒否、実際の位置・向きから求めた幅は生成と元の物理証明に合格。
初期幅のproducerを実際の固定waypoint投影へ変更し、native473の局所地図回帰と
source106件が合格。Build116全26、tests103全2598も合格。
R474実APIと同じ保存解の生成・物理証明が合格。次は標準dev2-r76。
D2はStartで目標速度15→20km/hになった直後のdispatch2274で文脈を失効。
後の2285公開期限超過とは分離する。r75は未完走、元DLL・ユーザーJSON復元済み。
[現在の修正](../20260910-mpcc-empirical-plant/receiver-input-enclosure/initial-corridor-design.md)。

...
最新の実装検証はbuild123全26、package110全2618、source106合格。
停止後の新しいソルバー入力を、実際の停止判断と両初期化の識別子付きで保存する処理を追加。
R593旧3失敗/新3合格、R594過去4判断を保持。次はローカルcommit後の診断single-r9で
未取得の失敗入力を保存し、計画が補充されない原因を修正する。
[新しい記録の設計・検証](../20260910-mpcc-empirical-plant/receiver-input-enclosure/post-loss-source-observation-design.md)。

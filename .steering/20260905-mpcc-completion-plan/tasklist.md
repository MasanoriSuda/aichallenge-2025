...
## 2026-09-09 実適用の直接監査後の残作業

- [x] 実公開374と検査対象386を分けた930世界の再検証。
- [x] 観測専用DLLで受信・選択・適用の番号/時刻を直接取得。
- [x] 原本CIL、全438適用の選択identity、Domain照合を検証。
- [x] 七状態のwire=net仮定と既知抵抗componentの不一致をnativeで再現。
- [x] 新933世界の元の371/932→933対と方式比較を保存し、完遂計画を更新。
- [ ] prefix/QP/physical/Stop/publishの縦入力モデルを比較・統一する。
- [ ] packet上書き・可変適用phaseの予測/証明契約を閉じる。
- [ ] 新しいHEADでP3〜P4の全受入れ・提出を完了する。

最新順序は[更新計画](../20260909-mpcc-actuation-application/completion-plan.md)。
今回のnative不一致は修正前の失敗証拠であり、モデル修正済みのチェックではない。

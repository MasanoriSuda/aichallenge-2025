"""Bounded measured-footprint diagnostic; map and peer geometry acceptance remain open."""
...
root = Path(sys.argv[1] if len(sys.argv) > 1 else
            "output/20260908-footprint-single-r1")
assert re.fullmatch(r"output/[a-z0-9-]+", str(root))
root.mkdir(parents=True, exist_ok=False)
container_root = "/" + str(root)
name = sys.argv[2] if len(sys.argv) > 2 else "footprint-20260908-r1"
...
manifest = json.loads(Path(".steering/20260908-awsim-footprint-enclosure/manifest.json").read_text())
protocol = json.loads(Path(".steering/20260908-awsim-footprint-enclosure/protocol.json").read_text())
...
manifest.update(run_id=root.name, kind="single-six-lap-footprint-diagnostic",
...
manifest["build_log"] = "/tmp/mpcc-20260908-footprint-build.log"
manifest["tests"] = "output/20260908-awsim-footprint-enclosure/package-test-results.log"
...
manifest["acceptance_scope"] = (
    "Measured ego-body enclosure and coupled lateral accounting. Missing wall-map coverage and full peer enclosure "
...
Path(f".steering/20260908-awsim-footprint-enclosure/{root.name}-manifest.json").write_text(

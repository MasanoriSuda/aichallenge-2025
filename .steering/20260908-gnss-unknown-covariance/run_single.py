"""Bounded GNSS calibration diagnostic; geometry acceptance remains open."""
...
root = Path(sys.argv[1] if len(sys.argv) > 1 else
            "output/20260908-gnss-calibration-single-r1")
assert re.fullmatch(r"output/[a-z0-9-]+", str(root))
root.mkdir(parents=True, exist_ok=False)
container_root = "/" + str(root)
name = sys.argv[2] if len(sys.argv) > 2 else "gnss-calibration-20260908-r1"
...
manifest.update(run_id=root.name, kind="single-six-lap-gnss-calibration-diagnostic",
...
manifest["build_log"] = "/tmp/mpcc-20260908-gnss-covariance-build.log"
manifest["tests"] = "output/20260908-mpcc-stop-reference-single-r3/gnss-after-test-results.log"
...
manifest["acceptance_scope"] = (
    "Sensor variance intervention only. Known missing wall-map coverage and body extent mismatch "
...
Path(f".steering/20260908-gnss-unknown-covariance/{root.name}-manifest.json").write_text(

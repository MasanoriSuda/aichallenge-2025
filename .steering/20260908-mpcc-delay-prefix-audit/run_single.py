"""Bounded published-artifact capture; existing failure remains unresolved."""
...
root = Path(sys.argv[1] if len(sys.argv) > 1 else
            "output/20260908-execution-evidence-single-r1")
assert re.fullmatch(r"output/[a-z0-9-]+", str(root))
root.mkdir(parents=True, exist_ok=False)
container_root = "/" + str(root)
name = sys.argv[2] if len(sys.argv) > 2 else "execution-evidence-20260908-r1"
...
manifest = json.loads(Path(".steering/20260908-mpcc-delay-prefix-audit/run-manifest-template.json").read_text())
protocol = json.loads(Path(".steering/20260908-mpcc-delay-prefix-audit/run-protocol.json").read_text())
...
for item in manifest["files"]:
    ...
    item["preserved_path"] = str(preserved)
manifest.update(run_id=root.name, kind="single-six-lap-execution-evidence-diagnostic",
...
manifest["build_log"] = "/tmp/mpcc-20260908-execution-evidence-v2-build.log"
manifest["tests"] = "output/20260908-execution-evidence-v2-validation/package-test-results.log"
...
for relative in [
    ...
    path = Path(relative)
    preserved = root / "inputs" / relative
    manifest["files"].append(dict(path=relative, sha256=hashlib.sha256(path.read_bytes()).hexdigest(), preserved_path=str(preserved)))
    preserved.parent.mkdir(parents=True, exist_ok=True)
...
manifest["acceptance_scope"] = (
    "Observe actual previously published source/artifact at authority loss. No controller repair or integrated "
    "acceptance inferred from a single run; peer geometry remains unresolved.")
...
Path(f".steering/20260908-mpcc-delay-prefix-audit/{root.name}-manifest.json").write_text(

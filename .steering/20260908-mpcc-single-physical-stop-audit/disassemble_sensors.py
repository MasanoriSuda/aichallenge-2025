...
selected = {
    'ROS2Utility': {'UnityToRosRotation', 'UnityToRosPosition'},
    'ImuSensor': {'FixedUpdate'},
    'ImuRos2Publisher': {'Publish'},
    'NavsatfixRos2Publisher': {'Publish', 'ConvertMgrsToGeodetic', 'ConvertHeight'},
    '<PublishWithDelayAndNoise>d__24': {'MoveNext'},
}
lines = []
for type_row in pe.net.mdtables.TypeDef:
    for method in type_row.MethodList:
        if str(method.row.Name) not in selected.get(str(type_row.TypeName), set()):
            continue
...
Path('output/20260908-mpcc-stop-reference-single-r3/unity-sensor-cil.txt').write_text(
    '\n'.join(lines) + '\n')

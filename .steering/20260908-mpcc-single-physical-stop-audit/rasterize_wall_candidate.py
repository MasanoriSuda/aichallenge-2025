...
Not a production map: all heights are projected, and faces with abs(normal.z)
...
original = np.array(Image.open(map_root / config['image']))
result = original.copy()
height, width = original.shape
origin = np.array(config['origin'][:2])
resolution = config['resolution']
assert config['origin'][2] == 0 and config['negate'] == 0
mesh = np.load(root / 'unity-track-collision.npz')

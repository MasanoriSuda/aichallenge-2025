...
uri = str(root / 'first-failure-partial-bag/failure-prefix.mcap')
...
result = dict(uri=uri, source_window_sec=[244, 256],
              columns={topics[0]: ['source', 'receive', 'speed', 'acceleration', 'wire_steering'],
                       ...
                       topics[4]: ['source', 'receive', 'x', 'y', 'z', 'roll', 'pitch', 'yaw']}, rows=rows)
(root / 'first-failure-motion.json').write_text(json.dumps(result, indent=2) + '\n')

"""One fixed-binary six-lap Stop reference acceptance trial."""
...
root = Path(sys.argv[1] if len(sys.argv) > 1 else
            "output/20260908-mpcc-stop-reference-single-r1")
assert re.fullmatch(r"output/[a-z0-9-]+", str(root))
root.mkdir(parents=True, exist_ok=False)
container_root = "/" + str(root)
name = sys.argv[2] if len(sys.argv) > 2 else "mpcc-stop-reference-20260908-r1"
...
protocol = json.loads(Path(".steering/20260908-mpcc-stop-reference-feasibility/protocol.json").read_text())
...
manifest.update(run_id=root.name, kind="single-six-lap-stop-reference-acceptance",
                working_tree_patch_sha256=hashlib.sha256(patch).hexdigest(),
                termination="six-lap finish or720s deadline; record all penalties and authority failures")
(root / "binaries").mkdir()
...
manifest["build_log"] = "/tmp/mpcc-20260908-stop-reference-build.log"
manifest["tests"] = "output/20260908-mpcc-stop-reference-feasibility/package-test-results.log"
...
Path(f".steering/20260908-mpcc-stop-reference-feasibility/{root.name}-manifest.json").write_text(
...
termination = "720s acceptance deadline"
summary = {}
try:

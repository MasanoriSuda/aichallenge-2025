...
domain = sys.argv[2]
lines = re.sub(r"\x1b\[[0-9;]*m", "", (root / domain / "autoware.log").read_text(errors="replace")).splitlines()
decisions = []
callbacks = []
...
for name in ("result-summary.json", domain+"-result-details.json"):
    path = root / domain / name
    if path.exists():
        result[name] = json.loads(path.read_text())
(root / (domain+"-log-summary.json")).write_text(json.dumps(result, indent=2) + "\n")
print(json.dumps({k: v for k, v in result.items() if k not in (
    "supervisor_override_decisions", "recovery_transitions", "laps", "result-summary.json")}, indent=2))

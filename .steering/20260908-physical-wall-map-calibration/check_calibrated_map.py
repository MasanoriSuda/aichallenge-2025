...
source_yaml = package / 'env/final_ver3/occupancy_grid_map.yaml'
config = yaml.safe_load(source_yaml.read_text())
config['image'] = 'original-occupancy-grid.pgm'
config.pop('preserve_occupied_cells', None)
original_yaml = root / 'original-map.yaml'
original_yaml.write_text(yaml.safe_dump(config))
candidate_yaml = source_yaml

...
for type_row in pe.net.mdtables.TypeDef:
    for method in type_row.MethodList:
        if str(type_row.TypeName) not in {'VehicleRosInput','Vehicle','VehicleAccelInputGuard','ImuSensor','ImuRos2Publisher','VehicleReportRos2Publisher'} or not method.row.Rva:
            continue
...
Path('output/20260909-actuation-response-r1/unity-plant-cil.txt').write_text(
    '\n'.join(lines) + '\n')

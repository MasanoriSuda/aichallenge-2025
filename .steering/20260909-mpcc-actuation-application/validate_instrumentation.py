...
out=Path('output/20260909-actuation-instrumentation-r2')
   ...
   if strip:
    for j,i in enumerate(ins):
     ...
     if i.mnemonic=='call' and isinstance(target,tuple) and 'MPCCInputObservation:' in str(target):
      name=target[1].split(':')[-1];count={'Receive':5,'Choose':4,'Apply':8}[name]
      start=j-count+1
...
assert not diff
assert [x['name'] for x in probes].count('Apply')==2 and len(probes)==4

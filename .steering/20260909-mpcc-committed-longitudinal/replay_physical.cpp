...
int main(int argc, char **argv) {
  ...
  if (result.exact_trajectory) {
    ...
    output["original_stop"]["wall"] = m::mpcc_rate_resolved_physical_wall::to_string(wall.outcome);
    output["original_stop"]["dynamic_clear"] = dynamic.valid && dynamic.clear;
    output["original_stop"]["dynamic_clearance_m"] = dynamic.minimum_clearance_m;
    output["original_stop"]["terminal_speed_mps"] = result.exact_trajectory->velocity_mps.back();
    output["original_stop"]["rest_tolerance_mps"] = std::max(1e-9,value.physical_global_tolerance);
    output["original_stop"]["certified_reason"] = certified::to_string(plan.reason);
    if (!plan.plan) throw std::runtime_error("original published Stop cannot be certified");
    ...
    // Offline causal counterfactual: same observation, same delay/steering model,
    // only already serialized acceleration controls at their declared origins.
    namespace prediction = m::mpc_state_prediction;
    const double delay=request.control_origin_sec-request.now_sec;
    const auto rho=prediction::infer_response_steering(
      request.current_speed_mps, config["current_yaw_rate_radps"].as<double>(),
      request.current_time_steering_rad,value.wheelbase_m,value.yaw_response_gain,
      .1,value.maximum_abs_steering_rad);
    if (!rho) throw std::runtime_error("response inference unavailable");
    const prediction::State2D initial_pose{world.current_pose.x_m,world.current_pose.y_m,world.current_pose.yaw_rad};
    const auto original_prediction=prediction::predict_accelerating_yaw_response(
      initial_pose,request.current_speed_mps,config["observed_acceleration_mps2"].as<double>(),
      rho->steering_rad,request.current_time_steering_rad,request.current_steering_rad,
      value.wheelbase_m,value.yaw_response_gain,value.yaw_response_time_constant_sec,delay);
    output["predictor_reconstruction"]["position_error_m"]=std::hypot(original_prediction.state.x-request.control_pose.x_m,original_prediction.state.y-request.control_pose.y_m);
    output["predictor_reconstruction"]["speed_error_mps"]=original_prediction.longitudinal_velocity_mps-request.control_origin_speed_mps;
    output["predictor_reconstruction"]["response_error_rad"]=original_prediction.response_steering_rad-request.current_response_steering_rad;
    prediction::LongitudinalResponseObserver observer(.9,.5);
    std::optional<prediction::LongitudinalResponseObservation> observation;
    std::ifstream events(config["observer_events_file"].as<std::string>());
    char kind; double stamp,input; bool found=false;
    while (events>>kind>>stamp>>input) {
      if (kind=='O') observation=observer.observe(stamp,std::abs(input));
      else if (kind=='C') {
        if (stamp==request.now_sec) {found=true;break;}
        if (!observer.record_published_command(stamp,stamp+delay,input))
          throw std::runtime_error("invalid serialized command in native replay");
      } else throw std::runtime_error("unknown observer input event");
    }
    if (!found || !observation || std::abs(observation->speed_mps-request.current_speed_mps)>1e-12)
      throw std::runtime_error("native observation does not match recorded failure");
    const auto intervals=observer.prediction_intervals(request.now_sec,delay);
    if (!intervals) throw std::runtime_error("native predictor history unavailable");
    const auto predicted=prediction::predict_piecewise_yaw_response_trajectory(
      initial_pose,request.current_speed_mps,rho->steering_rad,
      request.current_time_steering_rad,request.current_steering_rad,
      value.wheelbase_m,value.yaw_response_gain,value.yaw_response_time_constant_sec,*intervals);
    auto causal=request;causal.measured_to_control_path.clear();causal.measured_to_control_elapsed_sec.clear();
    for (const auto & sample : predicted) {
      causal.measured_to_control_path.push_back({sample.prediction.state.x,sample.prediction.state.y,sample.prediction.state.yaw});
      causal.measured_to_control_elapsed_sec.push_back(sample.elapsed_sec);
    }
    const double speed_now=predicted.back().prediction.longitudinal_velocity_mps;
    const double rho_now=predicted.back().prediction.response_steering_rad;
    output["native_observer"]["measured_filtered_acceleration_mps2"]=observation->filtered_measured_acceleration_mps2;
    output["native_observer"]["command_filtered_acceleration_mps2"]=observation->filtered_command_acceleration_mps2;
    output["native_observer"]["response_residual_mps2"]=observation->response_residual_mps2();
    causal.control_pose=causal.measured_to_control_path.back();
    causal.control_origin_speed_mps=speed_now;causal.current_response_steering_rad=rho_now;
    const auto frame=m::mpc_stage_geometry::sample_course_frame(current->source.wall_course_frame_knots,current->source.course_progress_origin_m);
    if (!frame) throw std::runtime_error("current source frame unavailable");
    const auto frenet=m::mpcc_execution_contract::project_planar_pose_to_frenet(
      {causal.control_pose.x_m,causal.control_pose.y_m,causal.control_pose.yaw_rad},
      {frame->x_m,frame->y_m,frame->heading_rad});
    if (!frenet) throw std::runtime_error("current source projection failed");
    causal.control_origin_physical_progress_m=current->source.course_progress_origin_m+frenet->lag_m;
    observe(causal,"matched_response_causal_counterfactual");
    output["matched_response_causal_counterfactual"]["control_pose_x_m"]=causal.control_pose.x_m;
    output["matched_response_causal_counterfactual"]["control_pose_y_m"]=causal.control_pose.y_m;
    output["matched_response_causal_counterfactual"]["control_pose_yaw_rad"]=causal.control_pose.yaw_rad;
    output["matched_response_causal_counterfactual"]["authority"]=false;
    // Sensitivity only: a future velocity report must never become a production input.
    // Keep the recorded pose, peers, command clock and all proof requirements fixed.
    if (config["future_observed_speed_mps"]) {
      auto sensitivity = request;
      sensitivity.control_origin_speed_mps = config["future_observed_speed_mps"].as<double>();
      observe(sensitivity,"hindsight_velocity_only_sensitivity");
    }
  }

"""Fixed single/dev2 acceptance for the committed longitudinal-response producer."""
...
root = Path('output/20260909-committed-longitudinal-'+mode+'-'+attempt)
assert not subprocess.check_output(['docker', 'ps', '-q']).strip(), 'Other containers running'
assert '100% tests passed, 0 tests failed out of 60' in Path('/tmp/mpcc-longitudinal-package-r2.log').read_text()
assert 'Summary: 2359 tests, 0 errors, 0 failures, 0 skipped' in Path('/tmp/mpcc-longitudinal-package-r2.log').read_text()
root.mkdir(parents=True, exist_ok=False)
template = json.loads(Path('.steering/20260909-mpcc-coordinate-consistency/run-manifest-template.json').read_text())
manifest = dict(run_id=root.name, kind='committed-longitudinal-'+mode+'-acceptance',
...
for kind, log in [('build','/tmp/mpcc-longitudinal-full-build-r2.log'), ('package','/tmp/mpcc-longitudinal-package-r2.log')]:

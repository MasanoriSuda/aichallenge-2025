...
out=Path('/output/20260909-complete-rest-4264-retimed-r2');out.mkdir(exist_ok=False)
...
cmd=[link[0],'-std=c++17','-O2','-I'+str(p/'include'),'-I'+str(p/'src'),'-I/usr/include/eigen3','-I'+str(out),str(s/'compare_retimed.cpp'),'/output/20260909-complete-rest-initial-domain/adapter_candidate.cpp','-o',str(out/'compare')]+['libmulti_purpose_mpc_ros_mpcc_rate_resolved_production_adapter.a','libmulti_purpose_mpc_ros_mpcc_rate_resolved_command_candidate.a','libmulti_purpose_mpc_ros_mpcc_rate_resolved_retained_revalidation.a','libmulti_purpose_mpc_ros_mpcc_latest_state_feedback.a','libmulti_purpose_mpc_ros_mpc_state_prediction.so','libmulti_purpose_mpc_ros_mpcc_rate_resolved_certified_plan.a']+link[link.index('-o')+2:]

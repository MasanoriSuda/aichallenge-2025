...
steering = Path('/task-steering/20260909-mpcc-complete-rest-candidates')
run = Path('/output/20260909-complete-stop-dev2-r1')
out = Path('/output/20260909-complete-rest-4264-execution')
...
world_path = next((run/'d1/mpcc_architecture_snapshots').glob('000000004264*/snapshot.yaml'))
...
config = dict(current_world=str(world_path),
    ...
    path_length_m=1000.0, current_speed_mps=8.718348824128569,
    current_time_steering_rad=0.092016, previous_published_command_age_sec=0.02,
    maximum_acceleration_mps2=1.37,
    limits='Current physical steering rounded from controller log to six decimals; exact longitudinal speed from same-run odometry matched by position; publication age from matching command stamp. Path length 1000m is unused non-wrap surrogate, no lap alignment here. Full stop trajectory is diagnostic only and never grants authority.')
...
command = [link[0], '-std=c++17','-O2','-I'+str(package/'include'),'-I'+str(package/'src'),'-I/usr/include/eigen3',
    str(steering/'replay_4264.cpp'),str(copy),'-o',str(out/'reconstruct_execution'),
...
original = run/'d1-executed-3695/snapshot.yaml'

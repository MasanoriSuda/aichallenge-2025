"""Single/dev2 acceptance after complete-rest candidate generation and all-peer guidance."""
...
root = Path('output/20260909-complete-rest-'+mode+'-'+attempt)
assert not subprocess.check_output(['docker', 'ps', '-q']).strip(), 'Other containers running'
assert '100% tests passed, 0 tests failed out of 60' in Path('/tmp/mpcc-complete-rest-package-final.log').read_text()
assert 'Summary: 2368 tests, 0 errors, 0 failures, 0 skipped' in Path('/tmp/mpcc-complete-rest-package-final.log').read_text()
root.mkdir(parents=True, exist_ok=False)
template = json.loads(Path('.steering/20260909-mpcc-coordinate-consistency/run-manifest-template.json').read_text())
manifest = dict(run_id=root.name, kind='complete-rest-candidate-'+mode+'-acceptance',
...
for kind, log in [('build','/tmp/mpcc-complete-rest-build-final.log'), ('package','/tmp/mpcc-complete-rest-package-final.log')]:
    shutil.copy2(log,root/(kind+'.log'))
patch = subprocess.check_output(['git','diff','--',str(package)])
(root/'mpcc-source.patch').write_bytes(patch)
manifest['control_baseline'] = '3c1cd03b'
manifest['scope'] = 'Free controls through terminal rest on declared maximum stage clock, all-peer Cartesian QP guidance and shared initial/iterated tangent domain; earlier failures including4264remain open'

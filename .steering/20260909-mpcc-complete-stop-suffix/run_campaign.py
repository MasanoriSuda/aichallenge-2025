"""Single/dev2 acceptance after Stop coordinate and complete-suffix proof repairs."""
...
root = Path('output/20260909-complete-stop-'+mode+'-'+attempt)
assert not subprocess.check_output(['docker', 'ps', '-q']).strip(), 'Other containers running'
assert '100% tests passed, 0 tests failed out of 60' in Path('/tmp/mpcc-complete-stop-package.log').read_text()
assert 'Summary: 2366 tests, 0 errors, 0 failures, 0 skipped' in Path('/tmp/mpcc-complete-stop-package.log').read_text()
root.mkdir(parents=True, exist_ok=False)
template = json.loads(Path('.steering/20260909-mpcc-coordinate-consistency/run-manifest-template.json').read_text())
manifest = dict(run_id=root.name, kind='complete-stop-proof-'+mode+'-acceptance',
...
for kind, log in [('build','/tmp/mpcc-complete-stop-full-build-final.log'), ('package','/tmp/mpcc-complete-stop-package.log')]:
    shutil.copy2(log,root/(kind+'.log'))
patch = subprocess.check_output(['git','diff','--',str(package)])
(root/'mpcc-source.patch').write_bytes(patch)
manifest['control_baseline'] = '8d6ebf8a'
manifest['scope'] = 'Stop projection preserves physical pose; complete current-state stopping suffix owns terminal proof; free-control candidate producer remains offline; earlier integrated failures remain open'

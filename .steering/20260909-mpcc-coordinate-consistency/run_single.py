"""Bounded single-vehicle acceptance of the corrected coordinate model."""
...
root = Path(sys.argv[1] if len(sys.argv) > 1 else
            "output/20260909-coordinate-single-r1")
assert re.fullmatch(r"output/[a-z0-9-]+", str(root))
root.mkdir(parents=True, exist_ok=False)
container_root = "/" + str(root)
name = sys.argv[2] if len(sys.argv) > 2 else "coordinate-model-20260909-r1"
...
manifest = json.loads(Path(".steering/20260909-mpcc-coordinate-consistency/run-manifest-template.json").read_text())
protocol = json.loads(Path(".steering/20260909-mpcc-coordinate-consistency/run-protocol.json").read_text())
...
manifest.update(run_id=root.name, kind="single-six-lap-coordinate-model-acceptance",
...
manifest["build_log"] = "output/20260909-coordinate-native/build.log"
manifest["tests"] = "output/20260909-coordinate-native/package.log"
...
manifest["acceptance_scope"] = (
    "Check coordinate-model repair, physical certification and final authority. No integrated "
...
Path(f".steering/20260909-mpcc-coordinate-consistency/{root.name}-manifest.json").write_text(

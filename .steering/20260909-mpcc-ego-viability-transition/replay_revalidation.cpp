...
int main(int argc, char **argv) {
  if (argc != 5) return 2;
  const std::string evidence_key = argv[4];
  const bool previous_input = evidence_key == "previous_accepted_revalidation_evidence";
  if (!previous_input && evidence_key != "revalidation_evidence") return 2;
  std::string detail;
  ...
  if (result.exact_trajectory) {
    ...
    const auto observation=failure_node[evidence_key];
    ...
    if (!previous_input && (request.decision_id!=failure->source.identity.source_context.decision_id ||
    ...
    if (previous_input && (request.decision_id >= failure->source.identity.source_context.decision_id ||
      request.now_sec > failure->source.identity.snapshot_sec || request.control_origin_sec > failure->source.control_prediction_origin_sec))
      throw std::runtime_error("previous observed input does not precede failure");
    if (observation["inspected_interaction_fingerprint"].as<std::uint64_t>() != record->interaction_fingerprint ||
      observation["inspected_artifact"]["source_sequence"].as<std::uint64_t>() != value.identity.sequence)
      throw std::runtime_error("inspected source substitution");
    output["observed_input_role"] = evidence_key;
    ...
    const auto other_key = previous_input ? "revalidation_evidence" : "previous_accepted_revalidation_evidence";
    const auto other = failure_node[other_key];
    if (other && other["status"].as<std::string>() == "present" &&
      other["inspected_plan_status"].as<std::string>() == "present" &&
      other["inspected_interaction_fingerprint"].as<std::uint64_t>() == record->interaction_fingerprint) {
      const auto other_request = mpcc_observation::read_request(other["request"], std::filesystem::path(argv[3]).parent_path());
      if (request.obstacles.observed_sec == request.now_sec && other_request.obstacles.observed_sec == other_request.now_sec) {
        auto hypothetical = request;
        hypothetical.obstacles = other_request.obstacles;
        if (!previous_input) {
          auto rows = output["ego_group_sensitivity"];
          for (int group = 0; group < 5; ++group) {
            auto hybrid = request;
            const char * name = "unknown";
            if (group == 0) {
              name = "previous-speeds";
              hybrid.current_speed_mps = other_request.current_speed_mps;
              hybrid.control_origin_speed_mps = other_request.control_origin_speed_mps;
            } else if (group == 1) {
              name = "previous-physical-steering-response";
              hybrid.current_time_steering_rad = other_request.current_time_steering_rad;
              hybrid.current_response_steering_rad = other_request.current_response_steering_rad;
            } else if (group == 2) {
              name = "previous-command-steering-and-age";
              hybrid.current_steering_rad = other_request.current_steering_rad;
              hybrid.previous_published_steering_rad = other_request.previous_published_steering_rad;
              hybrid.previous_published_command_age_sec = other_request.previous_published_command_age_sec;
            } else if (group == 3) {
              name = "previous-physical-poses-and-progress";
              hybrid.control_pose = other_request.control_pose;
              hybrid.control_origin_physical_progress_m = other_request.control_origin_physical_progress_m;
              hybrid.measured_to_control_path = other_request.measured_to_control_path;
              hybrid.measured_to_control_elapsed_sec = other_request.measured_to_control_elapsed_sec;
            } else {
              name = "previous-execution-observation-clock";
              const double shift = other_request.now_sec - request.now_sec;
              hybrid.now_sec = other_request.now_sec;
              hybrid.control_origin_sec = other_request.control_origin_sec;
              hybrid.obstacles.observed_sec = hybrid.now_sec;
              for (auto & peer : hybrid.obstacles.obstacles) {
                peer.circle.x_m += shift * peer.circle.velocity_x_mps;
                peer.circle.y_m += shift * peer.circle.velocity_y_mps;
              }
            }
            const auto sensitivity = retained::evaluate(hybrid);
            auto n = rows[name];
            n["reason"] = retained::to_string(sensitivity.reason);
            n["terminal_certified"] = sensitivity.terminal_stop_certified;
            n["terminal_minimum_dynamic_clearance_m"] = sensitivity.terminal_stop_minimum_dynamic_clearance_m;
            n["terminal_reference_attempts"] = sensitivity.terminal_stop_reference_attempts;
            n["meaning"] = "Named hybrid input sensitivity; not an actual observation or feasible transition; no authority";
          }
        }
        const double dt = request.now_sec - other_request.now_sec;
        for (auto & peer : hypothetical.obstacles.obstacles) {
          peer.circle.x_m += dt * peer.circle.velocity_x_mps;
          peer.circle.y_m += dt * peer.circle.velocity_y_mps;
        }
        hypothetical.obstacles.observed_sec = request.now_sec;
        const auto substituted = retained::evaluate(hypothetical);
        auto comparison = output["counterfactual_peer_only"];
        comparison["meaning"] = "Other observed constant-velocity peer field projected to this exact ego observation clock; no authority or actual-input claim";
        comparison["peer_input_role"] = other_key;
        comparison["peer_projection_sec"] = dt;
        comparison["reason"] = retained::to_string(substituted.reason);
        comparison["proof_available"] = substituted.proof.has_value();
        comparison["terminal_stop_certified"] = substituted.terminal_stop_certified;
        comparison["terminal_minimum_dynamic_clearance_m"] = substituted.terminal_stop_minimum_dynamic_clearance_m;
        comparison["continuation_minimum_dynamic_clearance_m"] = substituted.minimum_dynamic_clearance_m;
      }
    }

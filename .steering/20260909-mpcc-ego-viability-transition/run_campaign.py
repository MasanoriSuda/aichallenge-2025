"""Single/dev2 observation after exact previous-accepted request capture."""
...
root = Path('output/20260909-ego-viability-'+mode+'-'+attempt)
assert not subprocess.check_output(['docker', 'ps', '-q']).strip(), 'Other containers running'
assert '100% tests passed, 0 tests failed out of 60' in Path('/tmp/mpcc-ego-viability-ekf-package.log').read_text()
assert 'Summary: 2372 tests, 0 errors, 0 failures, 0 skipped' in Path('/tmp/mpcc-ego-viability-ekf-package.log').read_text()
root.mkdir(parents=True, exist_ok=False)
template = json.loads(Path('.steering/20260909-mpcc-coordinate-consistency/run-manifest-template.json').read_text())
manifest = dict(run_id=root.name, kind='ekf-single-epoch-'+mode,
...
for kind, log in [('build','/tmp/mpcc-ego-viability-ekf-build-r2.log'), ('package','/tmp/mpcc-ego-viability-ekf-package.log')]:
...
manifest['control_baseline'] = 'b19d6d7c'
manifest['scope'] = 'Participant EKF fork from matching historical headers: one callback epoch for prediction, measurement delays and state publication; TF uses pose stamp. Node interfaces and filter/noise/smoothing unchanged. MPCC source b19d6d7c unchanged. Full race acceptance pending.'
manifest['new_package_source_evidence'] = 'All aichallenge_ekf_localizer files individually copied/hashed in files; tracked-only patch does not contain this untracked package.'

"""Unchanged bounded architecture comparisons on the immutable 945 world."""
from pathlib import Path
import hashlib,json,subprocess

out=Path('/output/20260909-exact-join-945-comparison');out.mkdir(exist_ok=False)
...
(out/'manifest.json').write_text(json.dumps(dict(baseline_commit='466bba5e',source=str(source),source_sha256=hashlib.sha256(source.read_bytes()).hexdigest(),program_sha256=hashlib.sha256(program.read_bytes()).hexdigest(),limitation='Fresh independent candidates, never substitutes for recorded361or alternate371. All-method failure is Unknown without a physical infeasibility proof.',results=results),indent=2)+'\n')

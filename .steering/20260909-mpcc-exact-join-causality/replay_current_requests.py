...
run = Path('/output/20260909-exact-join-dev2-r1')
assert (run/'monitor-summary.json').exists(), 'Simulator must finish first'
out = Path('/output/20260909-exact-join-current-revalidation-r1')
out.mkdir(exist_ok=False)
binary = Path('/output/20260909-final-authority-revalidation-tool-r2/replay')
...
for domain in ['d1', 'd2']:
    for path in sorted((run/domain/'mpcc_architecture_snapshots').glob('*/snapshot.yaml')):
        ...
        observation = document.get('revalidation_evidence', {})
        ...
        if row['status'] != 'present' or row['inspected_status'] != 'present':
            continue
        destination = out/domain/path.parent.name
        ...
        command = [str(binary), str(source), str(destination/'result.yaml'), str(path)]

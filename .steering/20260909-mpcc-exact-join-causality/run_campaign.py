"""Single/dev2 acceptance after GNSS cumulative heading repair."""
...
root = Path('output/20260909-exact-join-'+mode+'-'+attempt)
assert not subprocess.check_output(['docker', 'ps', '-q']).strip(), 'Other containers running'
assert '100% tests passed, 0 tests failed out of 60' in Path('/tmp/mpcc-exact-join-gnss-package.log').read_text()
assert 'Summary: 2371 tests, 0 errors, 0 failures, 0 skipped' in Path('/tmp/mpcc-exact-join-gnss-package.log').read_text()
root.mkdir(parents=True, exist_ok=False)
template = json.loads(Path('.steering/20260909-mpcc-coordinate-consistency/run-manifest-template.json').read_text())
manifest = dict(run_id=root.name, kind='gnss-heading-'+mode+'-acceptance',
...
for kind, log in [('build','/tmp/mpcc-exact-join-gnss-build.log'), ('package','/tmp/mpcc-exact-join-gnss-package.log')]:
...
manifest['control_baseline'] = '466bba5e'
manifest['scope'] = 'GNSS heading anchor owns cumulative subthreshold motion per node; sensor lever arm now follows accepted heading. No MPCC solver/control/proof limit change. Fresh single and coupled acceptance; full coverage remains open.'

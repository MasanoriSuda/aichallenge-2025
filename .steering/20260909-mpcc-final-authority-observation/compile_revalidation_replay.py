...
out = Path('/output/20260909-final-authority-revalidation-tool-' + sys.argv[1])
...
(out/'manifest.json').write_text(json.dumps(dict(command=cmd, return_code=result.returncode,
    ...
    purpose='Exact captured rejected Request and inspected artifact; zero solves; no authority'), indent=2)+'\n')

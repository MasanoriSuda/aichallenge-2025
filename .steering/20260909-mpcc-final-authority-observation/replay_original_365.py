...
source=Path('/output/20260909-final-authority-365-original/snapshot.yaml')
out=Path('/output/20260909-final-authority-365-replay');out.mkdir(exist_ok=False)
...
(out/'manifest.json').write_text(json.dumps(dict(command=cmd,return_code=r.returncode,source=str(source),source_sha256=hashlib.sha256(source.read_bytes()).hexdigest(),solver_invocations=0,authority=False,limits='Original observed world of actual365only;951current state and Stop379are not captured in the old run. This is not a current-world join.'),indent=2)+'\n')

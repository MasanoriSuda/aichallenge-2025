"""Single/dev2 acceptance after first-authority observation repair."""
...
root = Path('output/20260909-final-authority-'+mode+'-'+attempt)
assert not subprocess.check_output(['docker', 'ps', '-q']).strip(), 'Other containers running'
assert '100% tests passed, 0 tests failed out of 60' in Path('/tmp/mpcc-final-authority-package.log').read_text()
assert 'Summary: 2371 tests, 0 errors, 0 failures, 0 skipped' in Path('/tmp/mpcc-final-authority-package.log').read_text()
root.mkdir(parents=True, exist_ok=False)
template = json.loads(Path('.steering/20260909-mpcc-coordinate-consistency/run-manifest-template.json').read_text())
manifest = dict(run_id=root.name, kind='first-authority-observation-'+mode+'-acceptance',
...
for kind, log in [('build','/tmp/mpcc-final-authority-build.log'), ('package','/tmp/mpcc-final-authority-package.log')]:
    shutil.copy2(log,root/(kind+'.log'))
patch = subprocess.check_output(['git','diff','--',str(package)])
(root/'mpcc-source.patch').write_bytes(patch)
manifest['control_baseline'] = 'e5b8c455'
manifest['scope'] = 'Observation-only first-event recorder, separate final authority boundary and exact rejected Request/artifact capture. No solver, objective, control, proof or publisher change. Fixed dev2 is intended to observe the first actual moving loss; coupled race acceptance remains open.'

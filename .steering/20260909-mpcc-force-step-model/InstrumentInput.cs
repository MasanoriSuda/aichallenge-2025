...
public static class InstrumentInput {
  ...
  public static void Main(string[] args) {
    ...
    var forceProbe=probe.Types.Single(t=>t.Name=="MPCCForceObservation");
    Func<string,MethodReference> forceMethod=n=>module.ImportReference(forceProbe.Methods.Single(m=>m.Name==n));
    var physics=module.Types.Single(t=>t.Name=="Vehicle").Methods.Single(m=>m.Name=="FixedUpdate");
    var il=physics.Body.GetILProcessor();
    var first=physics.Body.Instructions.First();
    foreach(var i in new[]{Instruction.Create(OpCodes.Ldarg_0),Instruction.Create(OpCodes.Call,forceMethod("Begin"))})il.InsertBefore(first,i);
    var last=physics.Body.Instructions.Last();
    if(last.OpCode!=OpCodes.Ret)throw new Exception("unexpected physics exit");
    foreach(var i in new[]{Instruction.Create(OpCodes.Ldarg_0),Instruction.Create(OpCodes.Call,forceMethod("End"))})il.InsertBefore(last,i);
    var wheelForce=module.Types.Single(t=>t.Name=="Wheel" && t.Namespace=="AWSIM").Methods.Single(m=>m.Name=="UpdateWheelForce");
    var add=wheelForce.Body.Instructions.Last(i=>i.OpCode==OpCodes.Callvirt && ((MethodReference)i.Operand).Name=="AddForceAtPosition");
    Append(wheelForce.Body.GetILProcessor(),add,Instruction.Create(OpCodes.Ldarg_0),Instruction.Create(OpCodes.Ldloc_0),Instruction.Create(OpCodes.Ldloc_1),Instruction.Create(OpCodes.Call,forceMethod("Wheel")));
    LongBranches(physics);LongBranches(wheelForce);
    module.Write(args[2]);
    Console.WriteLine("Added Receive1/Choose1/Apply2/Begin1/End1/Wheel1 probes; original application data and branch targets preserved.");

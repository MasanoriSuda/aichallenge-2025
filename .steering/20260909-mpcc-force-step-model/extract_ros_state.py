...
manifest=[]
for domain in sorted(p.name for p in run.iterdir() if p.is_dir() and p.name in ['d1','d2','d3','d4']):
    ...
    while reader.has_next():
        ...
        elif topic.endswith('kinematic_state'):r.update(v=m.twist.twist.linear.x,vy=m.twist.twist.linear.y,yaw_rate=m.twist.twist.angular.z,x=m.pose.pose.position.x,y=m.pose.pose.position.y,z=m.pose.pose.position.z,frame_id=m.header.frame_id,child_frame_id=m.child_frame_id,orientation=dict(x=m.pose.pose.orientation.x,y=m.pose.pose.orientation.y,z=m.pose.pose.orientation.z,w=m.pose.pose.orientation.w))
        rows.append(r)

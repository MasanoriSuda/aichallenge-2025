...
manifest = dict(run_id=root.name, kind='longitudinal-model-input-observation-'+mode,
                baseline_commit=subprocess.check_output(['git','rev-parse','HEAD'],text=True).strip(),
                initial_conditions='standard '+mode+' with1lap/180s only; noNPCs/collisions on/handicap off/wall recovery off',
                seed='AWSIM default; explicit seed unavailable, no determinism claimed',
                termination='all vehicles finished, first active moving Emergency/Recovery, or180s host diagnostic deadline',
...
manifest['previous_scope'] = 'Broad single one-lap direct application observation for acceleration/braking/rest/steering model comparison. Participant remains1c4f377e; no gain/model/limit/solver change. Same four validated receiver probes asae2efe6a. Observation-only, not uninstrumented race/performance acceptance.'
manifest['scope'] = 'Read-only input/force/physics-step observation with seven generated CIL calls. Participant1c4f377e. One lap or first moving override; no performance acceptance or production model change.'
...
script = root/'dev-one-lap.sh'
script.write_text(original.replace('--laps unlimited','--laps 1').replace('--timeout 10000000.0','--timeout 180').replace('--lidar off', '--lidar off -logFile /'+str(root)+'/unity-player.log'))
overlay = root/'compose-one-lap.yml'
overlay.write_text('services:\n  simulator:\n    volumes:\n      - '+str((repo/script).resolve())+':/aichallenge/simulator_scripts/dev.sh:ro\n')
probe_root = Path('output/20260909-force-step-instrumentation-r4')
validation = json.loads((probe_root/'cil-validation-r2.json').read_text())
assert not validation['differences'] and len(validation['probe_calls']) == 7
...
summary = {}
termination = '180s host diagnostic deadline'
try:
    ...
    while time.monotonic() < deadline:
        ...
        if ('MPCC_INPUT_OBS error' in simulator_log or 'MPCC_FORCE_ERROR' in simulator_log or
            ...
            if vehicle_state == 'finish':
                try:
                    if source_result.stat().st_mtime >= manifest['launch_started_wall_sec']:
                        ...
                        if candidate.get('finished') and candidate.get('lap_count') == 1:

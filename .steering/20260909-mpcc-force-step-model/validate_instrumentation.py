...
out=Path('output/20260909-force-step-instrumentation-r4')
  ...
  for index,method in enumerate(t.MethodList):
   ...
   if strip:
    for j,i in enumerate(ins):
     ...
     if i.mnemonic=='call' and isinstance(target,tuple) and ('MPCCInputObservation:' in str(target) or 'MPCCForceObservation:' in str(target)):
      name=target[1].split(':')[-1];count={'Receive':5,'Choose':4,'Apply':8,'Begin':2,'End':2,'Wheel':4}[name]
      start=j-count+1

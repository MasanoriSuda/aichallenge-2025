...
for type_row in pe.net.mdtables.TypeDef:
    for method in type_row.MethodList:
        if str(type_row.TypeName) not in {'Vehicle','Wheel','Axle'} or not method.row.Rva:
            continue
...
Path('output/20260909-longitudinal-model-inputs-r1/unity-wheel-cil.txt').write_text(
    '\n'.join(lines) + '\n')

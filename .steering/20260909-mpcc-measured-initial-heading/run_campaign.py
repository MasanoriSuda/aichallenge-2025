"""Single/dev2 acceptance of measured initial heading and synchronized GNSS/IMU attitude."""
...
root = Path('output/20260909-measured-heading-'+mode+'-'+attempt)
assert not subprocess.check_output(['docker', 'ps', '-q']).strip(), 'Other containers running'
assert '100% tests passed, 0 tests failed out of 60' in Path('/tmp/mpcc-initial-heading-package-r1.log').read_text()
assert 'Summary: 2381 tests, 0 errors, 0 failures, 0 skipped' in Path('/tmp/mpcc-initial-heading-verified-r2.log').read_text()
assert 'Summary: 39 tests, 0 errors, 0 failures, 7 skipped' in Path('/tmp/mpcc-initial-heading-final-gnss-r4.log').read_text()
assert 'Summary: 11 tests, 0 errors, 0 failures, 0 skipped' in Path('/tmp/mpcc-initial-heading-focused-r3.log').read_text()
root.mkdir(parents=True, exist_ok=False)
template = json.loads(Path('.steering/20260909-mpcc-coordinate-consistency/run-manifest-template.json').read_text())
manifest = dict(run_id=root.name, kind='measured-initial-heading-'+mode,
...
for kind, log in [('build','/tmp/mpcc-initial-heading-build-r4.log'), ('launch-validation','/tmp/mpcc-initial-heading-launch-validation-r2.log'), ('initial-build','/tmp/mpcc-initial-heading-build-r1.log'), ('focused','/tmp/mpcc-initial-heading-focused-r3.log'), ('package','/tmp/mpcc-initial-heading-package-r1.log'), ('verification','/tmp/mpcc-initial-heading-verified-r2.log'), ('node-replay','/tmp/mpcc-initial-heading-node-replay-r4.log')]:
...
manifest['scope'] = 'Simulation IMU TF corrected to measured +pi/2; same-source GNSS/IMU attitude before lever arm, shared measured initial pose retaining source stamp. Old raw-IMU fallback removed. Control formulation, gains, covariance, bounds and solver unchanged. Full integrated acceptance pending.'

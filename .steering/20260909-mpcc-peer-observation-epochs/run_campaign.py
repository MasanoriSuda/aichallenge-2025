"""Bounded diagnostic after source-free physical-plan observation repair."""
...
root = Path('output/20260909-peer-epochs-'+mode+'-'+attempt)
assert not subprocess.check_output(['docker', 'ps', '-q']).strip(), 'Other containers running'
assert '100% tests passed, 0 tests failed out of 60' in Path('/tmp/mpcc-peer-epochs-package.log').read_text()
assert 'Summary: 2374 tests, 0 errors, 0 failures, 0 skipped' in Path('/tmp/mpcc-peer-epochs-package.log').read_text()
root.mkdir(parents=True, exist_ok=False)
template = json.loads(Path('.steering/20260909-mpcc-coordinate-consistency/run-manifest-template.json').read_text())
manifest = dict(run_id=root.name, kind='peer-epochs-observation-'+mode,
...
for kind, log in [('build','/tmp/mpcc-peer-epochs-build.log'), ('package','/tmp/mpcc-peer-epochs-package.log')]:
...
manifest['control_baseline'] = '36c2a01a'
manifest['scope'] = 'Observation-only complete physical plan/artifact capture without solver-source requirement. No controller, authority, model, gain, margin or solver changes. Obtain actual derived Stop evidence; coupled acceptance remains open.'

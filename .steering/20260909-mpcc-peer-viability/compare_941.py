"""Unchanged bounded architecture comparisons on the immutable 941 world."""
from pathlib import Path
import hashlib,json,subprocess

out=Path('/output/20260909-peer-viability-941-comparison');out.mkdir(exist_ok=False)
source=next(Path('/output/20260909-exact-join-dev2-r1/d1/mpcc_architecture_snapshots').glob('000000000941*normal-authority*/snapshot.yaml'))
...
(out/'manifest.json').write_text(json.dumps(dict(baseline_commit='9d76066c',source=str(source),source_sha256=hashlib.sha256(source.read_bytes()).hexdigest(),program_sha256=hashlib.sha256(program.read_bytes()).hexdigest(),limitation='Fresh independent candidates, never substitutes for recorded370or alternate371. All-method failure is Unknown without a physical infeasibility proof.',results=results),indent=2)+'\n')

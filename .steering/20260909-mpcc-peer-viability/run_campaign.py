...
root = Path('output/20260909-peer-viability-'+mode+'-'+attempt)
assert not subprocess.check_output(['docker', 'ps', '-q']).strip(), 'Other containers running'
assert '100% tests passed, 0 tests failed out of 60' in Path('/tmp/mpcc-peer-viability-package.log').read_text()
assert 'Summary: 2372 tests, 0 errors, 0 failures, 0 skipped' in Path('/tmp/mpcc-peer-viability-package.log').read_text()
root.mkdir(parents=True, exist_ok=False)
template = json.loads(Path('.steering/20260909-mpcc-coordinate-consistency/run-manifest-template.json').read_text())
manifest = dict(run_id=root.name, kind='previous-accepted-observation-'+mode,
...
for kind, log in [('build','/tmp/mpcc-peer-viability-build.log'), ('package','/tmp/mpcc-peer-viability-package.log')]:
    shutil.copy2(log,root/(kind+'.log'))
patch = subprocess.check_output(['git','diff','--',str(package),'aichallenge/workspace/src/aichallenge_submit/racing_kart_gnss_poser'])
(root/'mpcc-source.patch').write_bytes(patch)
manifest['control_baseline'] = '9d76066c'
manifest['scope'] = 'Observation only: exact ordinary accepted Request and rejected Request atomically captured with separate clocks and grids. Existing publication ledger remains authoritative. No solver/control/proof limit change. Full acceptance open.'

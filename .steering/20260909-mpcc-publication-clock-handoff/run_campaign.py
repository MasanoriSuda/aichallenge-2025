"""Single/dev2 acceptance after published clock handoff repair."""
...
root = Path('output/20260909-publication-clock-'+mode+'-'+attempt)
assert not subprocess.check_output(['docker', 'ps', '-q']).strip(), 'Other containers running'
assert '100% tests passed, 0 tests failed out of 60' in Path('/tmp/mpcc-publication-clock-package.log').read_text()
assert 'Summary: 2373 tests, 0 errors, 0 failures, 0 skipped' in Path('/tmp/mpcc-publication-clock-package.log').read_text()
root.mkdir(parents=True, exist_ok=False)
template = json.loads(Path('.steering/20260909-mpcc-coordinate-consistency/run-manifest-template.json').read_text())
manifest = dict(run_id=root.name, kind='publication-clock-handoff-'+mode,
...
for kind, log in [('build','/tmp/mpcc-publication-clock-build.log'), ('package','/tmp/mpcc-publication-clock-package.log')]:
...
manifest['control_baseline'] = '17bcf24b'
manifest['scope'] = 'Actual published Bundle-to-exact transition inherits the clock used by its accepted proof and final command even for the same artifact. Ordinary same-plan clock, stale decision rejection and all controller limits preserved. Integrated acceptance pending.'

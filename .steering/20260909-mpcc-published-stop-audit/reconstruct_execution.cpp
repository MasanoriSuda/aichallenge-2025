...
int main(int argc, char **argv) {
  ...
  if (result.exact_trajectory) {
    ...
    auto causal=request;
    causal.measured_to_control_path={world.current_pose};
    causal.measured_to_control_elapsed_sec={0.0};
    auto pose_now=initial_pose; double speed_now=request.current_speed_mps, rho_now=rho->steering_rad;
    std::vector<double> boundaries{request.now_sec,request.control_origin_sec};
    for (const auto & item : config["committed_acceleration_history"]) {
      const double origin=item["control_origin_sec"].as<double>();
      if (item["publication_observation_sec"].as<double>()>=request.now_sec)
        throw std::runtime_error("future input in causal replay");
      if (origin>request.now_sec && origin<request.control_origin_sec) boundaries.push_back(origin);
    }
    std::sort(boundaries.begin(),boundaries.end());
    boundaries.erase(std::unique(boundaries.begin(),boundaries.end()),boundaries.end());
    for (size_t i=0;i+1<boundaries.size();++i) {
      const double begin=boundaries[i]-request.now_sec,end=boundaries[i+1]-request.now_sec;
      double acceleration=std::numeric_limits<double>::quiet_NaN();
      for (const auto & item : config["committed_acceleration_history"])
        if (item["control_origin_sec"].as<double>()<=boundaries[i]+1e-12)
          acceleration=item["acceleration_mps2"].as<double>();
      if (!std::isfinite(acceleration)) throw std::runtime_error("incomplete committed history");
      const double steering_begin=request.current_time_steering_rad+(request.current_steering_rad-request.current_time_steering_rad)*begin/delay;
      const double steering_end=request.current_time_steering_rad+(request.current_steering_rad-request.current_time_steering_rad)*end/delay;
      const auto part=prediction::predict_accelerating_yaw_response_trajectory(
        pose_now,speed_now,acceleration,rho_now,steering_begin,steering_end,
        value.wheelbase_m,value.yaw_response_gain,value.yaw_response_time_constant_sec,end-begin);
      for (size_t j=1;j<part.size();++j) {
        causal.measured_to_control_path.push_back({part[j].prediction.state.x,part[j].prediction.state.y,part[j].prediction.state.yaw});
        causal.measured_to_control_elapsed_sec.push_back(begin+part[j].elapsed_sec);
      }
      pose_now=part.back().prediction.state;speed_now=part.back().prediction.longitudinal_velocity_mps;rho_now=part.back().prediction.response_steering_rad;
    }
    ...
    observe(causal,"committed_acceleration_causal_counterfactual");
    output["committed_acceleration_causal_counterfactual"]["control_pose_x_m"]=causal.control_pose.x_m;
    output["committed_acceleration_causal_counterfactual"]["control_pose_y_m"]=causal.control_pose.y_m;
    output["committed_acceleration_causal_counterfactual"]["control_pose_yaw_rad"]=causal.control_pose.yaw_rad;
    output["committed_acceleration_causal_counterfactual"]["authority"]=false;

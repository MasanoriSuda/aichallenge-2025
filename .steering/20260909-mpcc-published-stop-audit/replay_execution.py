...
steering=Path('/task-steering/20260909-mpcc-published-stop-audit')
run=Path('/output/20260909-published-stop-observation-dev2-r1')
out=run/'executed-stop-replay'
out.mkdir(exist_ok=True)
link=shlex.split((build/'CMakeFiles/mpcc_architecture_compare.dir/link.txt').read_text())
command=[link[0],'-std=c++17','-O2','-I'+str(package/'include'),'-I'+str(package/'src'),'-I/usr/include/eigen3',str(steering/'reconstruct_execution.cpp'),'-o',str(out/'reconstruct_execution'),'libmulti_purpose_mpc_ros_mpcc_rate_resolved_retained_revalidation.a','libmulti_purpose_mpc_ros_mpcc_latest_state_feedback.a',str(package/'src/mpc_state_prediction.cpp'),'libmulti_purpose_mpc_ros_mpcc_rate_resolved_certified_plan.a']+link[link.index('-o')+2:]

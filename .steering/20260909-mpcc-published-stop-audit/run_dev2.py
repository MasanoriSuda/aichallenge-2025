"""One unchanged-control diagnostic to capture the published Stop at authority loss."""
...
root = Path('output/20260909-published-stop-observation-dev2-r1')
assert not subprocess.check_output(['docker', 'ps', '-q']).strip(), 'Other containers running'
assert '100% tests passed, 0 tests failed out of 60' in Path('/tmp/mpcc-published-stop-observation-package.log').read_text()
root.mkdir(parents=True, exist_ok=False)
template = json.loads(Path('.steering/20260909-mpcc-coordinate-consistency/run-manifest-template.json').read_text())
manifest = dict(run_id=root.name, kind='observation-only-dev2-published-stop-loss',
...
for kind, log in [('build','/tmp/mpcc-published-stop-observation-build.log'), ('package','/tmp/mpcc-published-stop-observation-package.log')]:
...
shutil.copy2(__file__,root/'run_dev2.py')
    ...
    try:
        with (root/'down.log').open('w') as log:
            jobs = [subprocess.Popen(['docker','compose','-p',project,'down','--remove-orphans'],
                     env=environment,stdout=log,stderr=subprocess.STDOUT) for project in ['1','2']]

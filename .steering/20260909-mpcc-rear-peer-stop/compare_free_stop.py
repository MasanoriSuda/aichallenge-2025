...
out=Path('/output/20260909-rear-peer-free-stop-comparison');out.mkdir(exist_ok=False)
link=shlex.split((b/'CMakeFiles/mpcc_architecture_compare.dir/link.txt').read_text())
cmd=[link[0],'-std=c++17','-O2','-I'+str(p/'include'),'-I'+str(p/'src'),'-I/usr/include/eigen3',str(s/'compare_free_stop.cpp'),'-o',str(out/'compare')]+link[link.index('-o')+2:]

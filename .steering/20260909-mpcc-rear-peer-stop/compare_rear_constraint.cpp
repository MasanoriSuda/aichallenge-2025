...
int main(int argc,char **argv) {
  ...
  for (bool stop_intent : {false,true}) {
    ...
    for (auto topology : {d::LongitudinalTopology::Automatic,d::LongitudinalTopology::StayAhead}) {
      ...
      for(std::size_t sqp_depth : {0U,3U}) {
        ...
        const auto arm=c::evaluate_arm(c::Arm::PersistentA,candidate,record->interaction_fingerprint,fp,
          ...
          stop_intent ? c::TerminalStopLateralAuditMode::SolvedStopTrajectory : c::TerminalStopLateralAuditMode::NormalPathProfile);
        YAML::Node row;
        row["stop"]=stop_intent;row["topology"]=d::to_string(topology);row["sqp_depth"]=sqp_depth;

...
out=Path('/output/20260909-rear-peer-domain-comparison');out.mkdir(exist_ok=False)
adapter=Path('/output/20260909-rear-peer-linearization-domain/adapter_probe.cpp')
link=shlex.split((b/'CMakeFiles/mpcc_architecture_compare.dir/link.txt').read_text())
cmd=[link[0],'-std=c++17','-O2','-I'+str(p/'include'),'-I/usr/include/eigen3',str(s/'probe_domain_comparison.cpp'),str(adapter),'-o',str(out/'compare')]+link[link.index('-o')+2:]
...
(out/'manifest.json').write_text(json.dumps(dict(adapter_sha256=hashlib.sha256(adapter.read_bytes()).hexdigest(),world_sha256=hashlib.sha256(world.read_bytes()).hexdigest(),command=cmd,authority=False),indent=2)+'\n')

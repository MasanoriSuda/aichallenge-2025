"""Single/dev2 acceptance after coupled tangent-domain producer repair."""
...
root = Path('output/20260909-rear-peer-'+mode+'-'+attempt)
assert not subprocess.check_output(['docker', 'ps', '-q']).strip(), 'Other containers running'
assert '100% tests passed, 0 tests failed out of 60' in Path('/tmp/mpcc-rear-peer-package.log').read_text()
assert 'Summary: 2364 tests, 0 errors, 0 failures, 0 skipped' in Path('/tmp/mpcc-rear-peer-package.log').read_text()
root.mkdir(parents=True, exist_ok=False)
template = json.loads(Path('.steering/20260909-mpcc-coordinate-consistency/run-manifest-template.json').read_text())
manifest = dict(run_id=root.name, kind='coupled-tangent-domain-'+mode+'-acceptance',
...
for kind, log in [('build','/tmp/mpcc-rear-peer-full-build.log'), ('package','/tmp/mpcc-rear-peer-package.log')]:
    shutil.copy2(log,root/(kind+'.log'))
patch = subprocess.check_output(['git','diff','--',str(package)])
(root/'mpcc-source.patch').write_bytes(patch)
manifest['control_baseline'] = '4afa9948'
manifest['scope'] = 'adapter tangent input selected in immutable course transition domain; raw QP/model/parameters unchanged; prior rear-peer terminal failure remains open'

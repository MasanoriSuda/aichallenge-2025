...
Result diagnostic_evaluate_with_stop_profile(const Request &, const mpcc_rate_resolved_physical_adapter::StopLateralTargetProfile *);
}
...
int main(int argc, char **argv) {
  ...
  if (result.exact_trajectory) {
    ...
    output["candidate_scan"]=YAML::Node(YAML::NodeType::Sequence);
    const double local_start=request.control_origin_physical_progress_m-value.course_progress_origin_m;
    for (int target_index=-10;target_index<=30;++target_index) {
      const double target=target_index*.1;
      for (double ramp_length: {0.,1.,2.,4.}) {
        m::mpcc_rate_resolved_physical_adapter::StopLateralTargetProfile profile;
        // Explicitly declared new terminal geometry, never extrapolated. No
        // change to current actuation, dynamics, bounds, world or peer model.
        for (int j=0;j<=200;++j) {
          const double progress=record->source.wall_reference_progress_m.front()+
            (record->source.wall_reference_progress_m.back()-record->source.wall_reference_progress_m.front())*j/200.;
          const double f=ramp_length==0. ? 1. : std::clamp((progress-local_start)/ramp_length,0.,1.);
          profile.progress_m.push_back(progress);
          profile.lateral_m.push_back(current->source.request.initial_state[0]+f*(target-current->source.request.initial_state[0]));
        }
        const auto proof=retained::diagnostic_evaluate_with_stop_profile(request,&profile);
        YAML::Node row;
        row["target_lateral_m"]=target;row["ramp_length_m"]=ramp_length;
        row["reason"]=retained::to_string(proof.reason);row["accepted"]=proof.proof.has_value();
        row["terminal_reason"]=m::mpcc_rate_resolved_physical_adapter::to_string(proof.terminal_stop_reason);
        row["dynamic_clearance_m"]=proof.terminal_stop_minimum_dynamic_clearance_m;
        row["dynamic_blocker"]=proof.terminal_stop_blocking_obstacle_id;
        row["wall_valid"]=proof.terminal_stop_path_clearance.valid;
        row["wall_clear"]=proof.terminal_stop_path_clearance.clear;
        row["authority"]=false;
        if (proof.proof) {
          row["terminal_speed_mps"]=proof.proof->terminal_stop_trajectory.velocity_mps.back();
          row["terminal_progress_m"]=proof.proof->terminal_stop_trajectory.progress_m.back();
          row["terminal_lateral_m"]=proof.proof->terminal_stop_trajectory.lateral_m.back();
        }
        output["candidate_scan"].push_back(row);
      }
    }
  }

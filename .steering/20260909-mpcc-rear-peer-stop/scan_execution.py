...
steering = Path('/task-steering/20260909-mpcc-rear-peer-stop')
run = Path('/output/20260909-failure-bundle-dev2-r1')
out = Path('/output/20260909-rear-peer-stop-profile-scan')
...
instrumented = instrumented.replace('evaluate_with_stop_profile', 'diagnostic_evaluate_with_stop_profile').replace('static Result diagnostic_evaluate_with_stop_profile', 'Result diagnostic_evaluate_with_stop_profile')
copy = out/'retained_revalidation_instrumented.cpp'
copy.write_text(instrumented)
world_path = next((run/'d2/mpcc_architecture_snapshots').glob('000000001629*/snapshot.yaml'))
...
config = dict(current_world=str(world_path),
    ...
    path_length_m=1000.0, current_speed_mps=4.342925292866534,
    current_time_steering_rad=-0.198264, previous_published_command_age_sec=0.02,
    maximum_acceleration_mps2=1.37,
    limits='Current physical steering rounded from controller log to six decimals; current speed from exactly matching odometry pose/source stamp; publication age from matching command stamp. Path length 1000m is unused non-wrap surrogate, no lap alignment here. Full stop trajectory is diagnostic only and never grants authority.')
...
command = [link[0], '-std=c++17','-O2','-I'+str(package/'include'),'-I'+str(package/'src'),'-I/usr/include/eigen3',
    str(steering/'scan_execution.cpp'),str(copy),'-o',str(out/'reconstruct_execution'),
    'libmulti_purpose_mpc_ros_mpcc_latest_state_feedback.a','libmulti_purpose_mpc_ros_mpc_state_prediction.so',
...
original = run/'d2-executed-1100/snapshot.yaml'

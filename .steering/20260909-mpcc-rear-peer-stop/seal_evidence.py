...
repo = Path.cwd()
validation = Path('output/20260909-rear-peer-validation')
validation.mkdir(exist_ok=True)
for path in Path('/tmp').glob('mpcc-rear-peer-*.log'):
...
for directory in Path('output').glob('20260909-rear-peer-*'):
...
(steering/'evidence.json').write_text(json.dumps(dict(
    baseline_commit='4afa9948',
    scope='tangent domain repair accepted locally; free-controls-through-rest witnesses offline only; integrated dev2 rejected',
    build_packages=25,ctest_groups=60,colcon_records=2364,
    focused_native_cases=19,

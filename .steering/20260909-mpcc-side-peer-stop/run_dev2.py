"""One sealed dev2 acceptance after side-peer geometry and current-world Stop repairs."""
...
root = Path('output/20260909-side-peer-stop-dev2-r1')
assert not subprocess.check_output(['docker', 'ps', '-q']).strip(), 'Other containers running'
assert '100% tests passed, 0 tests failed out of 60' in Path('/tmp/mpcc-side-peer-package.log').read_text()
root.mkdir(parents=True, exist_ok=False)
template = json.loads(Path('.steering/20260909-mpcc-coordinate-consistency/run-manifest-template.json').read_text())
manifest = dict(run_id=root.name, kind='changed-input-dev2-six-lap-side-peer-stop',
...
for kind, log in [('build','/tmp/mpcc-side-peer-full-build.log'), ('package','/tmp/mpcc-side-peer-package.log')]:

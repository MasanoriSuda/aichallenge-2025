...
out=Path('/output/20260909-stop-provenance-timeline');out.mkdir(exist_ok=False)
reader=rosbag2_py.SequentialReader()
reader.open(rosbag2_py.StorageOptions(uri=str(run/'d2/rosbag2_autoware'),storage_id='mcap'),rosbag2_py.ConverterOptions('',''))
...
(out/'manifest.json').write_text(json.dumps(dict(run=str(run),domain=2,topics=topics,rows=len(rows),bag_sha256=hashlib.sha256((run/'d2/rosbag2_autoware/rosbag2_autoware_0.mcap').read_bytes()).hexdigest(),limits='No publisher solution ID in control_cmd. Bag receipt cannot recover controller steady receipt age exactly. No claim that selection implies publication.'),indent=2)+'\n')
for row in rows:
    if row['stamp']>=10.38:print(json.dumps(row),flush=True)

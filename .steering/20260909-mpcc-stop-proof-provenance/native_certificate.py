...
out=Path('/output/20260909-stop-provenance-native-'+sys.argv[1]);out.mkdir(exist_ok=False)
link=shlex.split((b/'CMakeFiles/test_mpcc_architecture_comparison.dir/link.txt').read_text())
sources=[p/'test/test_mpcc_architecture_comparison.cpp',p/'src/mpcc_rate_resolved_stop_lattice_shadow.cpp']
...
with (out/'test.log').open('w') as log:r=subprocess.run([str(out/'regression'),'--gtest_filter=*CurrentWorldStop*'],cwd=b,stdout=log,stderr=subprocess.STDOUT,timeout=120)

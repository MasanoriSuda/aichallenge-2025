"""Single/dev2 acceptance after solved-certificate provenance repair."""
...
root = Path('output/20260909-stop-provenance-'+mode+'-'+attempt)
assert not subprocess.check_output(['docker', 'ps', '-q']).strip(), 'Other containers running'
assert '100% tests passed, 0 tests failed out of 60' in Path('/tmp/mpcc-stop-provenance-package.log').read_text()
assert 'Summary: 2369 tests, 0 errors, 0 failures, 0 skipped' in Path('/tmp/mpcc-stop-provenance-package.log').read_text()
root.mkdir(parents=True, exist_ok=False)
template = json.loads(Path('.steering/20260909-mpcc-coordinate-consistency/run-manifest-template.json').read_text())
manifest = dict(run_id=root.name, kind='stop-certificate-provenance-'+mode+'-acceptance',
...
for kind, log in [('build','/tmp/mpcc-stop-provenance-build.log'), ('package','/tmp/mpcc-stop-provenance-package.log')]:
    shutil.copy2(log,root/(kind+'.log'))
patch = subprocess.check_output(['git','diff','--',str(package)])
(root/'mpcc-source.patch').write_bytes(patch)
manifest['control_baseline'] = 'ae6862aa'
manifest['scope'] = 'Stop physical proof binds the accepted solved artifact residual and reports source-validation failures; all candidate generation, objectives, physical bounds and validators preserved. Live923/925root cause remains open.'

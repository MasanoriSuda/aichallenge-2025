...
out = Path('/output/20260909-stop-viability-pair-tool-' + sys.argv[1])
...
cmd = [link[0], '-std=c++17', '-O2', '-I'+str(p/'include'), '-I'+str(p/'src'),
       '-I/usr/include/eigen3', str(s/'replay_pair.cpp'), '-o', str(out/'replay'),
       '-Wl,--start-group'] + archives + ['-Wl,--end-group', str(b/'libmulti_purpose_mpc_ros_mpc_state_prediction.so')] + dependencies
...
(out/'manifest.json').write_text(json.dumps(dict(command=cmd, return_code=result.returncode,
    files={str(s/name):hashlib.sha256((s/name).read_bytes()).hexdigest() for name in
           ('replay_pair.cpp','../20260909-mpcc-peer-observation-epochs/certified_plan_input.hpp')},
    purpose='Exact captured before/after Requests on their own clocks and inspected artifacts; zero solves; no authority'), indent=2)+'\n')

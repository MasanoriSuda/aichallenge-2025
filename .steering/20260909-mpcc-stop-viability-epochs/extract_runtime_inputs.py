...
out = Path('/output/20260909-stop-viability-runtime-inputs-r1')
...
for domain in ['d1', 'd2']:
    ...
    wanted = ['/v2x/vehicle_positions', '/localization/kinematic_state', '/control/command/control_cmd', '/vehicle/status/steering_status', '/clock']
    ...
    while reader.has_next():
        ...
        if not 0 <= sec:
            continue
        row = dict(topic=topic, stamp=sec, stamp_ns=stamp.sec*1000000000+stamp.nanosec, receipt=ns*1e-9)
        ...
        elif topic.endswith('steering_status'):
            row['measured_steering']=m.steering_tire_angle
        elif topic!='/clock':
            row.update(speed=m.longitudinal.speed, acceleration=m.longitudinal.acceleration,
                wire_steering=m.lateral.steering_tire_angle, wire_steering_rate=m.lateral.steering_tire_rotation_rate)
        rows.append(row)

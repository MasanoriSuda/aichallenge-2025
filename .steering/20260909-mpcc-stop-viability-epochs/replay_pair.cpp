#include "../20260909-mpcc-peer-observation-epochs/certified_plan_input.hpp"
...
int main(int argc,char **argv) {
  ...
  if(key!="publication_bundle") {
    ...
    // Counterfactual diagnostics only. Original replay and proof remain above.
    const auto other_key=key=="revalidation_evidence" ? "previous_accepted_revalidation_evidence" : "revalidation_evidence";
    const auto other_observation=root[other_key];
    mpcc_observation::require(other_observation["status"].as<std::string>()=="present","missing paired observation");
    const auto other_plan=mpcc_observation::read_source_free_plan(other_observation["certified_plan_evidence"],path.parent_path());
    mpcc_observation::require(m::mpcc_rate_resolved_execution_artifact::same_identity(plan->execution_artifact->identity,other_plan->execution_artifact->identity),"paired artifact mismatch");
    auto other=mpcc_observation::read_request(other_observation["request"],path.parent_path());
    auto peer_only=request;peer_only.obstacles=other.obstacles;
    const double dt=request.now_sec-other.now_sec;
    for(auto & peer:peer_only.obstacles.obstacles) {peer.circle.x_m+=dt*peer.circle.velocity_x_mps;peer.circle.y_m+=dt*peer.circle.velocity_y_mps;}
    peer_only.obstacles.observed_sec=request.now_sec;
    const auto peer_result=retained::evaluate(peer_only);
    auto comparison=output["other_peer_field_only"];
    comparison["projected_sec"]=dt;comparison["reason"]=retained::to_string(peer_result.reason);
    comparison["terminal_clearance_m"]=peer_result.terminal_stop_minimum_dynamic_clearance_m;
    comparison["solved_suffix"]=peer_result.terminal_stop_uses_solved_suffix;
    auto anchor_only=request;anchor_only.execution_clock=other.execution_clock;
    const auto anchor_result=retained::evaluate(anchor_only);
    output["other_clock_only"]["reason"]=retained::to_string(anchor_result.reason);
    output["other_clock_only"]["cursor_sec"]=anchor_result.cursor_elapsed_sec;
    output["other_clock_only"]["terminal_clearance_m"]=anchor_result.terminal_stop_minimum_dynamic_clearance_m;

    // Named estimator counterfactual: hold measured body speed/yaw rate only
    // from its source epoch to decision time; then keep the recorded actuator
    // prediction unchanged. SE(2) equivariance transforms its complete path.
    const auto matches=YAML::LoadFile(argv[4]);
    for(const auto & match:matches) {
      if(match["decision"].as<std::uint64_t>()!=request.decision_id)continue;
      const auto source=match["source"];
      mpcc_observation::require(source["x"].as<double>()==request.measured_to_control_path.front().x_m && source["y"].as<double>()==request.measured_to_control_path.front().y_m && source["v"].as<double>()==request.current_speed_mps,"raw source substitution");
      const double age=request.now_sec-source["stamp"].as<double>();
      const auto first=request.measured_to_control_path.front();
      const auto aligned=m::mpc_state_prediction::predict_constant_turn_rate(
        {first.x_m,first.y_m,first.yaw_rad},source["v"].as<double>(),source["yaw_rate"].as<double>(),age);
      const double angle=aligned.yaw-first.yaw_rad,c=std::cos(angle),s=std::sin(angle);
      const auto transform=[&](const m::recovery_footprint::Pose2D & p) {
        const double x=p.x_m-first.x_m,y=p.y_m-first.y_m;
        return m::recovery_footprint::Pose2D{aligned.x+c*x-s*y,aligned.y+s*x+c*y,std::atan2(std::sin(p.yaw_rad+angle),std::cos(p.yaw_rad+angle))};
      };
      auto held_twist=request;
      held_twist.control_pose=transform(request.control_pose);
      for(auto & pose:held_twist.measured_to_control_path)pose=transform(pose);
      const auto estimate=retained::evaluate(held_twist);
      const auto stop_estimate=retained::evaluate_stop_successor(held_twist);
      auto estimate_node=output["source_to_now_held_twist"];
      estimate_node["meaning"]="Counterfactual existing constant-twist estimator for observation age, then unchanged recorded actuator rollout. Raw safety monitor not replaced; measured speed/yaw held over age; historical scalar progress retained diagnostically. No accuracy/authority claim.";
      estimate_node["age_sec"]=age;
      estimate_node["shift_m"]=std::hypot(aligned.x-first.x_m,aligned.y-first.y_m);
      estimate_node["yaw_shift_rad"]=angle;
      estimate_node["reason"]=retained::to_string(estimate.reason);
      estimate_node["solved_suffix"]=estimate.terminal_stop_uses_solved_suffix;
      estimate_node["terminal_clearance_m"]=estimate.terminal_stop_minimum_dynamic_clearance_m;
      estimate_node["independent_stop"]=retained::to_string(stop_estimate.reason);
      estimate_node["independent_stop_clearance_m"]=stop_estimate.dynamic_clearance.minimum_clearance_m;
    }

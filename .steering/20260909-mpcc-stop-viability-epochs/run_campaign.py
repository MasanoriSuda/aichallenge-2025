"""Single/dev2 acceptance after explicit source-to-now motion estimation."""
...
root = Path('output/20260909-stop-viability-'+mode+'-'+attempt)
...
manifest = dict(run_id=root.name, kind='stop-viability-epochs-'+mode,
...
manifest['control_baseline'] = '4b2474da'
manifest['scope'] = 'Explicit source-to-now constant-twist motion estimation before unchanged 0.13s actuator prediction; independent canonical current pose and raw safety monitor. No authority, actuator model, gain, margin or solver changes. Full integrated acceptance pending.'

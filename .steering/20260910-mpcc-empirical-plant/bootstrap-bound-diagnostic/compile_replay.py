...
(out/'manifest.json').write_text(json.dumps(dict(command=cmd, return_code=result.returncode,
    ...
    purpose='Sealed dev2-r9 bound comparison and current/inspected production source builders; driver records pipeline invocations; no runtime authority'), indent=2)+'\n')

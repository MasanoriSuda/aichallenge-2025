...
(out/'manifest.json').write_text(json.dumps(dict(command=cmd, return_code=result.returncode,
    ...
    purpose='Prospective candidate packet binding on sealed dev2-r8, same Stop materialization/publication and snapshot roundtrip; zero solves; no authority'), indent=2)+'\n')

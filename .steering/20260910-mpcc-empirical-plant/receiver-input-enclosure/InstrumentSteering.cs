...
public static class InstrumentInput {
  ...
  public static void Main(string[] args) {
    ...
    var steering=module.Types.Single(t=>t.Name=="Vehicle").Fields.Single(f=>f.Name=="SteerAngleInput");
    var steerWrite=receive.Body.Instructions.Single(i=>i.OpCode==OpCodes.Stfld && i.Operand==steering);
    var steerLog=new System.Collections.Generic.List<Instruction>{Instruction.Create(OpCodes.Ldarg_0),
      Instruction.Create(OpCodes.Ldarg_1),Instruction.Create(OpCodes.Ldloc_0),Instruction.Create(OpCodes.Call,ros)};
    if(ros.ReturnType.IsValueType)steerLog.Add(Instruction.Create(OpCodes.Box,ros.ReturnType));
    steerLog.Add(Instruction.Create(OpCodes.Call,method("Steer")));
    Append(receive.Body.GetILProcessor(),steerWrite,steerLog.ToArray());
    LongBranches(receive);LongBranches(chosen);
    module.Write(args[2]);
    Console.WriteLine("Added Receive1/Choose1/Apply2/Steer1 probes; original application data and branch targets preserved.");

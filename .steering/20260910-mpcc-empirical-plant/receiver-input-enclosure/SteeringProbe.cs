...
public static class MPCCInputObservation {
  ...
  private static float WireSteering(object message) {
    var lateral=Member(message,"Lateral");
    foreach(var owner in new object[]{message,lateral}) {
      foreach(var name in new string[]{"Steering_tire_angle","steering_tire_angle","SteeringAngle","steeringAngle","Steer","steer"}) {
        var value=Member(owner,name); if(value!=null)return Convert.ToSingle(value);
      }
    }
    throw new InvalidOperationException("Missing original steering payload");
  }
  public static void Receive(object receiver,object message,float input) {
    try {
      ...
      Console.WriteLine("MPCC_INPUT_OBS rx id="+RuntimeHelpers.GetHashCode(receiver)+" seq="+s.Sequence+
        ...
        " wire_steering="+Text(WireSteering(message))+
  ...
  // Called directly after the original Ackermann steering field write.
  public static void Steer(object receiver, object message, float degrees, object rosTime) {
    try {
      var stamp=Member(message,"Stamp") ?? Member(Member(message,"Header"),"Stamp");
      var wire=WireSteering(message);
      Console.WriteLine("MPCC_INPUT_OBS steer id="+RuntimeHelpers.GetHashCode(receiver)+
        " source_ns="+Ns(stamp)+" ros_ns="+Ns(rosTime)+" wire_steering="+Text(wire)+
        " steer_degrees="+Text(degrees)+" mono_ticks="+Stopwatch.GetTimestamp()+
        " topic="+Member(receiver,"ackermannControlCommandTopic"));
    } catch(Exception e) {Console.WriteLine("MPCC_INPUT_OBS error steer "+e.GetType().Name);}
  }

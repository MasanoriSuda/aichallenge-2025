#!/bin/bash
set -euo pipefail
mpcc_probe_out=/output/20260911-steering-instrumentation-r2
...
mkdir "$mpcc_probe_out/compiler-packages"
cp /var/cache/apt/archives/*.deb "$mpcc_probe_out/compiler-packages/"
for mpcc_probe_deb in /var/cache/apt/archives/*.deb; do
    dpkg-deb -x "$mpcc_probe_deb" /
done
mcs -target:library -out:"$mpcc_probe_out/ObservationProbe.dll" /task-steering/20260910-mpcc-empirical-plant/receiver-input-enclosure/SteeringProbe.cs
mcs -r:/usr/lib/mono/gac/Mono.Cecil/0.11.0.0__0738eb9f132ed756/Mono.Cecil.dll -out:"$mpcc_probe_out/InstrumentInput.exe" /task-steering/20260910-mpcc-empirical-plant/receiver-input-enclosure/InstrumentSteering.cs

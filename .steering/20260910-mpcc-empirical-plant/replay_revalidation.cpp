  ...
  if (result.exact_trajectory) {
    ...
    if (other && other["status"].as<std::string>() == "present" &&
      ...
      if (request.obstacles.observed_sec == request.now_sec && other_request.obstacles.observed_sec == other_request.now_sec) {
        ...
        if (!previous_input) {
          auto hybrid = request;
          hybrid.execution_clock = other_request.execution_clock;
          const auto replay = retained::evaluate(hybrid);
          auto n = output["previous_publication_anchor_only"];
          n["reason"] = retained::to_string(replay.reason);
          n["cursor_elapsed_sec"] = replay.cursor_elapsed_sec;
          n["expected_steering_rad"] = replay.expected_steering_rad;
          n["terminal_certified"] = replay.terminal_stop_certified;
          n["terminal_minimum_dynamic_clearance_m"] = replay.terminal_stop_minimum_dynamic_clearance_m;
          n["meaning"] = "Only preserve previous accepted source publication anchor; all measured state, peer field and clocks unchanged. No authority.";
        }

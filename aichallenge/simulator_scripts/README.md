...
```
make simulator-<mode> / make dev / make dev2..dev4 / make gate1..gate3 / make e2e-single / make e2e-teacher / make e2e-npc-single / make e2e-peer-audit-mpc / make e2e-peer-audit-student
...
- `make dev` / `make gate1..gate3` / `make e2e-single` / `make e2e-teacher` / `make e2e-npc-single` / `make e2e-peer-audit-mpc` / `make e2e-peer-audit-student` / `make e2e` / `make e2e-final` は AWSIM に加えて Autoware も起動する複合ターゲット。

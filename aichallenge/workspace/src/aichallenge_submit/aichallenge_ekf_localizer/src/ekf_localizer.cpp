...
#include "aichallenge_ekf_localizer/covariance.hpp"
#include "aichallenge_ekf_localizer/mahalanobis.hpp"
#include "aichallenge_ekf_localizer/matrix_types.hpp"
#include "aichallenge_ekf_localizer/measurement.hpp"
#include "aichallenge_ekf_localizer/numeric.hpp"
#include "aichallenge_ekf_localizer/state_index.hpp"
#include "aichallenge_ekf_localizer/state_transition.hpp"
#include "aichallenge_ekf_localizer/string.hpp"
#include "aichallenge_ekf_localizer/warning.hpp"
#include "aichallenge_ekf_localizer/warning_message.hpp"
...
void EKFLocalizer::timerCallback()
{
  ...
  if (!pose_queue_.empty()) {
    ...
    for (size_t i = 0; i < n; ++i) {
      ...
      measurementUpdatePose(*pose, current_time);
    }
  ...
  if (!twist_queue_.empty()) {
    ...
    for (size_t i = 0; i < n; ++i) {
      ...
      measurementUpdateTwist(*twist, current_time);
    }
  ...
  current_ekf_pose_.header.stamp = current_time;
  ...
  current_ekf_twist_.header.stamp = current_time;
  current_ekf_twist_.twist.linear.x = vx;
  current_ekf_twist_.twist.angular.z = wz;

  /* publish ekf result */
  publishEstimateResult(current_time);
}
...
void EKFLocalizer::timerTFCallback()
{
  ...
  transform_stamped.header.stamp = current_ekf_pose_.header.stamp;
...
void EKFLocalizer::measurementUpdatePose(
  const geometry_msgs::msg::PoseWithCovarianceStamped & pose, const rclcpp::Time & current_time)
{
  ...
  const rclcpp::Time & t_curr = current_time;
...
void EKFLocalizer::measurementUpdateTwist(
  const geometry_msgs::msg::TwistWithCovarianceStamped & twist, const rclcpp::Time & current_time)
{
 ...
 */
void EKFLocalizer::publishEstimateResult(const rclcpp::Time & current_time)
{
  const Eigen::MatrixXd X = ekf_.getLatestX();

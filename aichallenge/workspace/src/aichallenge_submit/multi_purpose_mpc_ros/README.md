...
ロバスト壁warning中にfreshな同側Missionが得られない場合は、`v2x_overtake_runtime_wall_fallback_delay_sec`後に、同じpass sideと名目車間を維持した中央寄りgoalを`v2x_overtake_runtime_wall_center_contraction_max_adjustment`以内で生成し、ShiftOut/Pass/Return全体のpreflight成立時だけ原子的に置換します。縮退経路も成立せず、対象が`v2x_overtake_runtime_wall_return_min_front_distance`以上前方でReturn corridorが成立する場合だけ、Recovery速度へ落とさずReturnします。front capの初回解除にはロバスト車体sweepを要求しますが、解除後は実寸車体と実寸予測sweepが非重複である限り、ロバスト推奨余裕の境界揺れだけでは再適用しません。actual overlap、wall hard guard、target discontinuityは従来どおりhard条件です。

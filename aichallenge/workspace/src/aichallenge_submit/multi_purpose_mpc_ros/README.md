...
OvertakeLine有効時は対象`vehicle_id`とpass sideをlockします。`v2x_overtake_target_hold_sec`以内の一時欠落ではPassを維持し、対象が`v2x_overtake_line_return_clear_distance`以上後方にいる状態を`v2x_overtake_clear_confirm_sec`継続して初めてReturnへ入ります。minimum-motionの横目標は、validated corridorに余裕がある場合、`v2x_overtake_minimum_motion_preferred_clearance_buffer`の範囲で対象車から離す側へ寄せます（狭いcorridorでは幅の半分を上限とし、壁側の成立範囲は変更しません）。直線または現在カーブの外側では、完全なShiftOut/Pass/Returnと追加壁余裕を満たす候補がある場合だけ`v2x_overtake_minimum_motion_straight_outer_extra_clearance`を上乗せし、不成立なら通常bufferへ戻します。Passのfront capは`v2x_overtake_pass_front_overlap_lateral_clearance`で解除し、解除中だけ`v2x_overtake_pass_front_cap_reapply_lateral_clearance`までのヒステリシスを使います。再適用後は前者の解除閾値を再び満たす必要があり、前方衝突保護は常に前者の実横離隔を使います。予測footprintの一時的な重複は`v2x_overtake_pass_predicted_overlap_confirm_sec`で確認してからcapを戻します。固定corridor centerはwall feasible boundsと対象車の最低横離隔の共通範囲へ収めます。receding-horizonのrobust clearance縮退時も、`v2x_overtake_receding_horizon_min_target_surface_reserve`を車体実寸へ加えたexecution floorを下回りません。minimum-motionで既に横離隔を確保した状態では、`v2x_overtake_safe_separation_forward_escape_enabled`により、対象が設定された前方window内にいる間は後退側へ速度を落とさず前方完遂を優先します。横並びコミット後に対象が後方へ入り、現在車体が非重複、execution corridorが有効で実測前進進捗がfreshなら、予測sweepだけが保守的に重なっても同じ側のrear-clear完遂を維持します。対象が一度後方へ入った後、進捗停止と前後差の悪化が設定時間・距離を超えた場合は、`v2x_overtake_safe_separation_progress_loss_disengage_enabled`により横側を保持したまま対象より低い速度へ有限時間だけ切り替えます。対象が前方clearになってから既存のMission再検証へ移るため、横並び位置でReturnやRecoveryを開始しません。局所時間・距離上限に達しても、`v2x_overtake_safe_separation_progress_extension_enabled`が有効で、対象前方距離の改善量とfreshness、車体非重複、短期安全条件が成立する場合だけ局所枠を有限回再設定します。Pass全体の絶対時間・距離上限は再設定されません。soft abort後の同側再計画待ちは`v2x_overtake_safe_separation_soft_abort_replan_grace_sec`と距離上限による一回限りのリースです。また、一度十分な速度へ到達したforward escapeが低速かつ進捗停止になった場合は、`v2x_overtake_safe_separation_forward_stall_*`によりPassを保持し続けず再計画handoffへ移ります。前方window外、進捗なし、延長回数または絶対上限到達、現在車体の重複、壁不成立、emergency、solver異常はRecovery対象で、終了理由はログ上で個別表示します。Return直後の再取得は、安定した同一ID・同一side・gap/curve許可・時間/復帰進捗の全条件を満たす場合だけPassへ戻します。不明ID、position jump、timeout、連続solver失敗はRecovery側へ倒します。Recoveryの速度上限は専用flag/valueで、`v2x_follow_velocity`を暗黙には使いません。

ロバスト壁warning中にfreshな同側Missionが得られない場合は、`v2x_overtake_runtime_wall_fallback_delay_sec`後に、同じpass sideの中央寄りgoalを`v2x_overtake_runtime_wall_center_contraction_max_adjustment`以内で生成します。`v2x_overtake_runtime_wall_preplan_lookahead_sec`は再計画を始めるpreview horizonであり、予測TTCが`v2x_overtake_runtime_wall_preplan_action_ttc_sec`より大きい間は、代替prefix不成立だけでは現在の可行Missionを破棄しません。予測距離に余裕がある場合は`v2x_overtake_line_max_shift_distance`までcenterward shiftを延長し、横加速度を下げます。最初は名目対車間とロバスト壁余裕を使用し、現在車体が非重複でhard wall faultがない場合だけ、実寸対車間と`v2x_overtake_line_min_wall_clearance`まで段階的に縮退します。縮退候補にも既存のfootprint、横加速度、DP prefix preflightをすべて適用し、成立時だけ原子的にMissionを置換します。action帯で縮退経路も成立せず、対象が`v2x_overtake_runtime_wall_return_min_front_distance`以上前方でReturn corridorが成立する場合だけ、Recovery速度へ落とさずReturnします。front capの初回解除にはロバスト車体sweepを要求しますが、解除後は実寸車体と実寸予測sweepが非重複である限り、ロバスト推奨余裕の境界揺れだけでは再適用しません。actual overlap、wall hard guard、target discontinuityは従来どおりhard条件です。

左右に完全なMissionがある場合は、ShiftOut完了後からrear-clearまでに予測される対車surface clearanceと、壁・corridor・Returnの余裕の最小値も比較します。`v2x_overtake_side_future_interaction_min_advantage`以上の差があるときだけ広い側を優先し、僅差では従来のrear-clear進捗順位を維持します。また対象が選択側へ横移動中の場合、`v2x_overtake_body_clear_intrusion_margin_gain`と`v2x_overtake_body_clear_intrusion_margin_max_extra_sec`でbody-clear deadlineの時間余裕を増やします。対象が停止中または選択側から離れている場合は追加しないため、停止車両回避へ一律の長距離gateは追加しません。

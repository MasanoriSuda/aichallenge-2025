...
struct OvertakeLineConfig
{
  ...
  double pass_horizon_absolute_time_limit{10.0};
  ...
  double entry_retry_cooldown_sec{1.0};
  double follow_prepare_timeout_sec{4.0};
  double follow_prepare_max_distance{20.0};
  double completed_target_hold_sec{1.5};
...
struct MPC
{
    ...
    const double now_sec)
  {
    ...
    output.overtake_return_corridor_blocked = overtake_return_corridor_blocked;
    output.overtake_return_corridor_blocker_id =
      overtake_return_corridor_blocker_id;
    output.overtake_return_corridor_blocker_longitudinal =
      overtake_return_corridor_blocker_longitudinal;
      ...
      output.locked_target_longitudinal > -side_longitudinal_range)
    {
      ...
      output.overtake_entry_target_speed = output.locked_target_speed;
    }
    const bool committed_overtake_mission_active =
      overtake_line_state_.phase != OvertakeLinePhase::Idle;
    output.overtake_completed_target_entry_suppressed =
      overtake_core::should_suppress_completed_target_reacquire(
      overtake_core::CompletedTargetReacquireSuppressionRequest{
        !completed_overtake_target_block_.target_vehicle_id.empty(),
        output.target_vehicle_id == completed_overtake_target_block_.target_vehicle_id,
        committed_overtake_mission_active,
        now_sec,
        completed_overtake_target_block_.until_sec});
    ...
    // Follow owns clearance recovery. Once a validated ShiftOut/Pass starts, its bounded
    // stage-speed policy owns longitudinal separation; front-risk, deceleration and emergency
    // policies below remain active independently.
    ...
    const auto start_grid_breakout_side = start_grid_breakout_attempt ?
      start_grid_grace::resolve_breakout_side(
      start_grid_grace::BreakoutSideContext{
        model->spatial_state.e_y,
        nearest_front_lateral,
        cfg.v2x_behavior.start_grid_breakout_side_deadband,
        continuing_start_grid_breakout ? start_grid_breakout_side_sign_ : 0}) :
      start_grid_grace::BreakoutSideDecision{false, 0};
    // Once the target becomes laterally clear it leaves the generic front set, but the same
    // explicit ShiftOut/Pass line still owns the start-grid maneuver. Keep the breakout speed
    // release until rear-clear/line completion instead of falling back to front+closing speed.
    const bool start_grid_breakout_line_owned =
      start_grid_breakout_line_active && start_grid_breakout_line_target_matches &&
      output.locked_target_seen && !output.locked_target_position_jump;
    output.start_grid_breakout_active =
      continuing_start_grid_breakout || start_grid_breakout_line_owned;
    const bool committed_minimum_motion_corridor_active =
    ...
    // Behavior and OvertakeLine speed ownership share one confirmation clock.
    // Otherwise a transient predicted overlap preserves the front cap but
    // simultaneously drops the same committed Pass into SafetyBrake.
    overtake_line_state_.pass_predicted_overlap_since_sec =
      committed_predicted_overlap_confirmation.overlap_since_sec;
    const bool committed_predicted_overlap_confirmed =
      committed_predicted_overlap_confirmation_monitored ?
    ...
    update_start_grid_suppression_diagnostics(
      ...
      emergency_overrides_start_grid && !start_grid_breakout_attempt,
      output.target_vehicle_id, nearest_front_distance, nearest_front_speed,
      front_risk.required_decel);
    if (emergency_overrides_start_grid && !start_grid_breakout_attempt) {
      ...
      !start_grid_breakout_attempt)
    {
      ...
      output.state = V2XBehaviorState::SafetyBrake;
      output.reason = front_risk_reason(
        "front risk emergency", front_risk, effective_front_risk_level);
      return commit_v2x_behavior_state(output, now_sec);
    }
    ...
    if (low_speed_avoidance_gap_blocked) {
      ...
      if (apply_follow_velocity_limits(
          ...
          stopped_front_risk, stopped_front_risk_level, false, front_risk_applied)) {
    ...
    const double static_mission_validation_distance =
      1.25 * std::max(0.5, cfg.v2x_behavior.overtake_line.shift_distance) +
      std::max(
...
private:
    ...
    const int pass_side_sign, const std::string & reason)
  {
    ...
    overtake_line_state_.pending_side_replan_since_sec =
      std::numeric_limits<double>::quiet_NaN();
    overtake_line_state_.early_side_replanned = false;
  ...
  void replan_early_shiftout_side(
    const int next_side, const double now_sec, const double current_ey,
    const std::optional<double> & corridor_goal_ey)
  {
    const int previous_side = overtake_line_state_.pass_side_sign;
    if (
      overtake_line_state_.phase != OvertakeLinePhase::ShiftOut ||
      next_side == 0 || next_side == previous_side)
    {
      return;
    }
    if (cfg.v2x_behavior.overtake_line.debug_log_enabled) {
      RCLCPP_INFO(
        rclcpp::get_logger("mpc_controller"),
        "OvertakeLine early side replan: side=%d -> %d, ey=%.2f, wp_id=%d",
        previous_side, next_side, current_ey, model->wp_id);
    }
    overtake_line_state_.pass_side_sign = next_side;
    overtake_locked_side_sign_ = next_side;
    overtake_line_state_.fixed_pass_corridor_goal_ey = corridor_goal_ey;
    overtake_line_state_.phase_start_sec = now_sec;
    overtake_line_state_.phase_start_ey = current_ey;
    overtake_line_state_.target_ey = current_ey;
    overtake_line_state_.phase_traveled_m = 0.0;
    ...
    const std::optional<overtake_core::OvertakeMissionCandidate> & selected_mission)
  {
    ...
    if (selected_mission.has_value()) {
      const auto pass_plan = overtake_core::build_overtake_pass_plan(
        overtake_core::OvertakePassPlanRequest{
          ...
          0.0});
      if (pass_plan.valid) {
        overtake_line_state_.mission_plan = pass_plan;
      } else {
        RCLCPP_ERROR(
          rclcpp::get_logger("mpc_controller"),
          "OvertakeLine selected mission could not be frozen as PassPlan: "
          "target=%s, side=%d, goal=%.2f",
          overtake_line_state_.target_vehicle_id.c_str(),
          selected_mission->pass_side_sign,
          selected_mission->goal_lateral_m);
      }
    }
    // Start-grid breakout has its own validated corridor and does not carry a
    // normal mission candidate. Normal overtake execution requires the atomic
    // PassPlan above.
    overtake_line_state_.mission_path_frozen =
    ...
    overtake_line_state_.mission_shift_distance =
      ...
      std::max(0.5, selected_mission->shift_distance_m) :
      std::max(0.5, line_cfg.shift_distance);
    overtake_line_state_.mission_closing_speed_limit =
      selected_mission.has_value() &&
      std::isfinite(selected_mission->closing_speed_mps) ?
      std::clamp(
    ...
    overtake_line_state_.mission_path_total_distance =
      ...
      (selected_mission.has_value() &&
      std::isfinite(selected_mission->return_distance_m) ?
      std::max(0.5, selected_mission->return_distance_m) :
    ...
    overtake_line_state_.mission_static_valid_until_pass_m =
      selected_mission.has_value() &&
      std::isfinite(selected_mission->static_valid_until_pass_m) ?
      std::max(0.0, selected_mission->static_valid_until_pass_m) :
      overtake_line_state_.mission_pass_hold_distance;
    ...
    overtake_line_state_.mission_prediction_source_age_sec =
      selected_mission.has_value() ? selected_mission->prediction_source_age_sec :
      std::numeric_limits<double>::infinity();
    overtake_line_state_.mission_prediction_epoch_sec =
      selected_mission.has_value() ? selected_mission->prediction_epoch_sec :
      -std::numeric_limits<double>::infinity();
    overtake_line_state_.mission_prediction_horizon_sec =
      selected_mission.has_value() ?
      std::max(0.0, selected_mission->prediction_horizon_sec) : 0.0;
    overtake_line_state_.mission_dynamic_valid_until_sec =
      selected_mission.has_value() ? selected_mission->dynamic_valid_until_sec :
      -std::numeric_limits<double>::infinity();
    overtake_line_state_.mission_predicted_rear_clear_time_sec =
      selected_mission.has_value() ? selected_mission->predicted_rear_clear_time_sec :
      std::numeric_limits<double>::infinity();
    const double pass_origin_shift_distance =
      selected_mission.has_value() && selected_mission->direct_pass ? 0.0 :
      overtake_line_state_.mission_shift_distance;
    overtake_line_state_.mission_predicted_rear_clear_pass_m =
      selected_mission.has_value() &&
      std::isfinite(selected_mission->predicted_rear_clear_ego_distance_m) ?
      std::max(
        0.0,
        selected_mission->predicted_rear_clear_ego_distance_m -
        pass_origin_shift_distance) :
    ...
    if (overtake_line_state_.mission_plan.has_value()) {
      ...
      overtake_line_state_.fixed_pass_corridor_goal_ey =
        pass_plan.path.pass_lateral_m;
      overtake_line_state_.mission_shift_distance =
        pass_plan.path.shift_distance_m;
      overtake_line_state_.mission_pass_hold_distance =
      ...
      overtake_line_state_.mission_path_total_distance =
        pass_plan.path.shift_distance_m + pass_plan.path.pass_distance_m +
        pass_plan.path.return_distance_m;
    ...
    std::nullopt) const
  {
    ...
    for (int i = 0; i < N; ++i) {
      ...
      double required_lateral_accel =
        2.0 * lateral_shift / (time_to_target * time_to_target);
      if (
        max_lateral_accel > kEps &&
        required_lateral_accel > max_lateral_accel)
      {
        const double direction = target_ey >= current_ey ? 1.0 : -1.0;
        const double limited_shift =
          0.5 * max_lateral_accel * time_to_target * time_to_target;
    ...
    const Eigen::VectorXd & lb, const Eigen::VectorXd & ub, const double now_sec)
  {
    ...
    const auto try_refresh_committed_pass_horizon =
      [&](const bool longitudinal_only, std::string & failure_reason) {
      ...
      if (overtake_line_state_.pass_horizon_safe_separation_active) {
        ...
        if (safe_separation.action == overtake_core::SafeSeparationAction::KeepSameSide) {
          ...
          transition_overtake_line_phase(
            OvertakeLinePhase::Return, now_sec, current_ey,
            overtake_line_state_.pass_side_sign,
            "SafeSeparation rear-clear; returning");
      ...
      if (!overtake_line_state_.pass_horizon_safe_separation_active) {
        ...
        if (horizon_action == overtake_core::PassHorizonAction::Return) {
          transition_overtake_line_phase(
            OvertakeLinePhase::Return, now_sec, current_ey,
            overtake_line_state_.pass_side_sign,
            "rear-clear confirmed within committed Pass horizon");
        } else if (
          horizon_action ==
          overtake_core::PassHorizonAction::RequestSameSideExtension)
        {
          std::string extension_failure_reason;
          if (!try_refresh_committed_pass_horizon(false, extension_failure_reason)) {
          ...
          overtake_core::PassHorizonAction::RequestLongitudinalRefresh)
        {
          ...
          if (!try_refresh_committed_pass_horizon(true, refresh_failure_reason)) {
            if (!begin_safe_separation(
    ...
    if (!horizon_evaluation.execution_feasible())
    {
        ...
        !horizon_evaluation.static_map_physical_infeasible_during_execution)
      {
        transition_overtake_line_phase(
          OvertakeLinePhase::Return, now_sec, current_ey,
          overtake_line_state_.pass_side_sign,
          "locked target rear; returning before static wall recovery");
  ...
  bool overtake_guard_allows(
    ...
    const bool start_grid_breakout,
    std::string & block_reason) const
  {
    ...
    if (
      !start_grid_breakout && min_front_distance > kEps &&
    ...
    if (
      !start_grid_breakout && guard_phase.require_prepare_distance &&
      min_prepare_distance > kEps &&
    ...
    if (cfg.v2x_behavior.overtake_guard_reachable_gap_enabled) {
      ...
      if (
        !start_grid_breakout && min_gap_time > kEps &&
        reachable_gap.first_gap_time < min_gap_time)
      {
        std::ostringstream ss;
      ...
      if (
        !start_grid_breakout && max_lateral_accel > kEps &&
    ...
    std::ostringstream ss;
    ss << (start_grid_breakout ?
      "start-grid breakout gap available, first_s=" :
      "front vehicle and guarded gap available, first_s=")
    ...
    const double current_ey, const double lower, const double upper, std::string & block_reason) const
  {
    ...
    // Close-follow is only a geometric side-target fallback. It must not bypass the normal
    // new-line front-distance contract; the 20260724 d1 run otherwise began ShiftOut at
    // 2.99 m while both karts were still accelerating at the same rate.
    const auto entry_guard = v2x_overtake_core::resolve_overtake_guard_phase(
      v2x_overtake_core::OvertakeGuardPhaseRequest{
        false,
        cfg.v2x_behavior.overtake_guard_min_front_distance,
        cfg.v2x_behavior.overtake_continue_min_front_distance});
    const double min_front_distance = std::max({
      0.0,
      cfg.v2x_behavior.overtake_close_follow_min_front_distance,
      entry_guard.min_front_distance_m});
  ...
  double overtake_stage_speed_ceiling(
    const double front_distance, const double front_speed, const double entry_speed,
    const bool pass_phase, const bool lateral_clearance_latched,
    const double remaining_shiftout_distance) const
  {
    // Once the Pass phase has established persistent physical lateral
    // separation, the locked target is no longer a longitudinal obstacle.
    // Keep all global/risk limits, but do not model the early-pass closing cap
    // as an attainable-speed ceiling for completion checks.
    if (pass_phase && lateral_clearance_latched) {
      return std::numeric_limits<double>::infinity();
    }
    if (
      !cfg.v2x_behavior.overtake_stage_speed_enabled ||
      !std::isfinite(front_speed) || front_speed < 0.0 ||
      !std::isfinite(entry_speed) || entry_speed < 0.0)
    {
      return std::numeric_limits<double>::infinity();
    }

    double closing_speed_limit = overtake_mission_closing_speed_limit();
    closing_speed_limit = v2x_overtake_core::resolve_unlatched_pass_closing_speed(
      closing_speed_limit,
      cfg.v2x_behavior.overtake_pass_unlatched_max_closing_speed,
      pass_phase, lateral_clearance_latched);
    bool adaptive_closing_applied = false;
    if (
      !pass_phase &&
      cfg.v2x_behavior.overtake_shiftout_adaptive_closing_speed_enabled &&
      std::isfinite(front_distance) && front_distance >= 0.0 &&
      std::isfinite(remaining_shiftout_distance) && remaining_shiftout_distance >= 0.0)
    {
      const auto adaptive_closing =
        ...
        v2x_overtake_core::AdaptiveShiftOutClosingSpeedRequest{
          ...
          cfg.v2x_behavior.overtake_shiftout_adaptive_min_time_sec});
      closing_speed_limit = adaptive_closing.closing_speed_mps;
      adaptive_closing_applied = true;
    }

    const double front_based_cap =
      std::max(0.0, front_speed) + closing_speed_limit;
    const double entry_speed_floor = adaptive_closing_applied ?
      std::min(entry_speed, front_based_cap) : entry_speed;
    return v2x_overtake_core::resolve_overtake_speed_reference(
      v2x_overtake_core::OvertakeSpeedReferenceRequest{
        v2x_overtake_core::OvertakeSpeedStage::ShiftOut,
        cfg.v_max,
        cfg.v_max,
        std::max(0.0, front_speed),
        entry_speed_floor,
        closing_speed_limit}).reference_speed_mps;
  }
  ...
  bool apply_follow_velocity_limits(
    ...
    const FrontRiskLevel front_risk_level, const bool suppress_generic_follow_cap,
    bool & front_risk_applied) const
  {
  ...
  V2XBehaviorOutput commit_v2x_behavior_state(V2XBehaviorOutput output, const double now_sec)
  {
    ...
    const bool overtake_execution_committed =
      overtake_line_state_.phase == OvertakeLinePhase::ShiftOut ||
      overtake_line_state_.phase == OvertakeLinePhase::Pass ||
      overtake_line_state_.phase == OvertakeLinePhase::FollowPrepare;
    ...
    if (final_state == V2XBehaviorState::Overtake) {
      ...
      if (output.start_grid_breakout_active) {
        // The same cycle has already validated an all-vehicle-inflated side corridor and the
        // execution zone. Release the race reference at entry instead of spending the first
        // hairpin at front+1.2 m/s while waiting for the ShiftOut phase label to change to Pass.
        // Domain/global speed caps and MPC a_max remain unchanged.
        const bool validated_breakout = start_grid_grace::should_preserve_breakout_line(
          start_grid_grace::BreakoutLineContext{
            output.start_grid_breakout_active,
            final_state == V2XBehaviorState::Overtake,
            output.overtake_gap_available,
            output.overtake_zone_allows});
        const auto breakout_speed =
          v2x_overtake_core::resolve_start_grid_breakout_speed_reference(
          v2x_overtake_core::StartGridBreakoutSpeedReferenceRequest{
            validated_breakout,
            cfg.v_max,
            cfg.v_max,
            std::isfinite(output.front_speed) ? std::max(0.0, output.front_speed) : 0.0,
            overtake_entry_speed_.value(),
            std::max(0.0, cfg.v2x_behavior.overtake_shiftout_max_closing_speed)});
        output.overtake_front_cap_release_ready = validated_breakout;
        output.desired_velocity = breakout_speed.reference_speed_mps;
        output.overtake_speed_front_cap_applied = breakout_speed.front_cap_applied;
        output.overtake_shiftout_adaptive_speed_applied = false;
      } else if (cfg.v2x_behavior.overtake_stage_speed_enabled) {
        ...
        if (std::isfinite(overtake_target_speed)) {
          ...
          const bool physical_shiftout_stage =
            !output.overtake_committed_pass_active;
          const double selected_or_mission_closing_speed =
            overtake_line_state_.mission_path_frozen ?
            overtake_mission_closing_speed_limit() :
            ...
            std::clamp(
              ...
              std::max(0.0, cfg.v2x_behavior.overtake_shiftout_max_closing_speed)) :
            overtake_mission_closing_speed_limit();
          double closing_speed_limit = selected_or_mission_closing_speed;
          closing_speed_limit = overtake_core::resolve_unlatched_pass_closing_speed(
            closing_speed_limit,
            cfg.v2x_behavior.overtake_pass_unlatched_max_closing_speed,
            output.overtake_committed_pass_active,
            ...
            std::isfinite(output.front_distance))
          {
            ...
            const auto adaptive_closing =
              ...
              v2x_overtake_core::AdaptiveShiftOutClosingSpeedRequest{
                std::min(
                  cfg.v2x_behavior.overtake_shiftout_min_closing_speed,
                  selected_or_mission_closing_speed),
                selected_or_mission_closing_speed,
    ...
    if (cfg.v2x_behavior.debug_log_enabled) {
      ...
      if (log_now) {
        RCLCPP_INFO(
          ...
          "lookahead_inner=%d, inner_pref=%d, "
          ...
          output.overtake_left_quality,
          output.overtake_right_quality,
          output.overtake_lookahead_inner_side,
          output.overtake_inner_preference_selected ? 1 : 0,
          output.overtake_selected_side_conflict ? 1 : 0,
...
struct StuckRecoveryAdapterConfig
{
  ...
  // After this many consecutive ForwardDurationLimit retries, require one
  // statically/V2X checked Reverse candidate. Zero disables the alternation.
  std::size_t aggressive_forward_retry_limit_before_reverse{0U};
...
Config load_config(const std::string & path)
{
  ...
  if (recovery) {
    ...
    if (maneuver) {
      ...
      out.aggressive_retry_min_lateral_change_m =
        maneuver["aggressive_retry_min_lateral_change_m"] ?
        maneuver["aggressive_retry_min_lateral_change_m"].as<double>() :
        out.aggressive_retry_min_lateral_change_m;
      out.aggressive_retry_min_heading_change_rad =
        maneuver["aggressive_retry_min_heading_change_rad"] ?
        maneuver["aggressive_retry_min_heading_change_rad"].as<double>() :
        out.aggressive_retry_min_heading_change_rad;
      ...
      adapter.adaptive_reverse_retry.enabled =
        maneuver["adaptive_reverse_retry_enabled"] ?
        maneuver["adaptive_reverse_retry_enabled"].as<bool>() : false;
      adapter.adaptive_reverse_retry.multiplier =
        maneuver["adaptive_reverse_retry_multiplier"] ?
        maneuver["adaptive_reverse_retry_multiplier"].as<double>() :
        adapter.adaptive_reverse_retry.multiplier;
      adapter.adaptive_reverse_retry.maximum_target_distance_m =
        maneuver["adaptive_reverse_retry_max_distance_m"] ?
        maneuver["adaptive_reverse_retry_max_distance_m"].as<double>() :
        adapter.adaptive_reverse_retry.maximum_target_distance_m;
      adapter.adaptive_reverse_retry.reset_forward_distance_m =
        maneuver["adaptive_retry_reset_forward_distance_m"] ?
        maneuver["adaptive_retry_reset_forward_distance_m"].as<double>() :
        adapter.adaptive_reverse_retry.reset_forward_distance_m;
      ...
      adapter.escape_confirm_distance_tolerance_m =
        ...
        adapter.escape_confirm_distance_tolerance_m;
      adapter.aggressive_forward_retry_limit_before_reverse =
        maneuver["aggressive_forward_retry_limit_before_reverse"] ?
        maneuver["aggressive_forward_retry_limit_before_reverse"].as<std::size_t>() :
        adapter.aggressive_forward_retry_limit_before_reverse;
    ...
    if (rejoin) {
      ...
      out.rejoin_lateral_regression_margin_m = rejoin["lateral_regression_margin_m"] ?
        rejoin["lateral_regression_margin_m"].as<double>() :
        out.rejoin_lateral_regression_margin_m;
    ...
    if (adapter.adaptive_reverse_retry.enabled && !core.simulation_only) {
      throw std::runtime_error(
              "stuck_recovery adaptive Reverse retry requires simulation_only: true");
    }
    const stuck_recovery::AdaptiveReverseRetryTracker adaptive_reverse_retry_validator(
      adapter.adaptive_reverse_retry);
    (void)adaptive_reverse_retry_validator;
    ...
    if (
      ...
      adapter.reverse_escape_distance_m ||
      adapter.adaptive_reverse_retry.maximum_target_distance_m >=
      core.supervisor.max_reverse_distance_m)) ||
  ...
  cfg.mpc.v2x_behavior.overtake_minimum_motion_inner_lookahead_distance = std::max(
    0.0,
    mpc["v2x_overtake_minimum_motion_inner_lookahead_distance"] ?
    mpc["v2x_overtake_minimum_motion_inner_lookahead_distance"].as<double>() : 0.0);
  cfg.mpc.v2x_behavior.overtake_minimum_motion_inner_min_open_distance = std::max(
    0.0,
    mpc["v2x_overtake_minimum_motion_inner_min_open_distance"] ?
    mpc["v2x_overtake_minimum_motion_inner_min_open_distance"].as<double>() : 0.0);
  cfg.mpc.v2x_behavior.overtake_minimum_motion_inner_min_corridor_width = std::max(
    0.0,
    mpc["v2x_overtake_minimum_motion_inner_min_corridor_width"] ?
    mpc["v2x_overtake_minimum_motion_inner_min_corridor_width"].as<double>() : 0.0);
  ...
  cfg.mpc.v2x_behavior.overtake_unseparated_front_reserve_distance = std::max(
    0.0,
    mpc["v2x_overtake_unseparated_front_reserve_distance"] ?
    mpc["v2x_overtake_unseparated_front_reserve_distance"].as<double>() : 0.0);
  ...
  cfg.mpc.v2x_behavior.overtake_pass_front_overlap_lateral_clearance = std::max(
    ...
    mpc["v2x_overtake_pass_front_overlap_lateral_clearance"] ?
    mpc["v2x_overtake_pass_front_overlap_lateral_clearance"].as<double>() : 0.0);
  const double effective_overtake_front_cap_release_clearance = std::max(
    cfg.mpc.v2x_behavior.overtake_pass_front_overlap_lateral_clearance,
    cfg.mpc.v2x_gap.vehicle_radius + cfg.mpc.v2x_gap.prediction_margin);
  cfg.mpc.v2x_behavior.overtake_pass_front_cap_reapply_lateral_clearance = std::min(
  ...
  cfg.mpc.v2x_behavior.overtake_pass_predicted_overlap_confirm_sec = std::max(
    ...
    mpc["v2x_overtake_pass_predicted_overlap_confirm_sec"].as<double>() : 0.0);
  ...
  cfg.mpc.v2x_behavior.start_grid_breakout_enabled =
    mpc["v2x_start_grid_breakout_enabled"] ?
    mpc["v2x_start_grid_breakout_enabled"].as<bool>() : false;
  cfg.mpc.v2x_behavior.start_grid_breakout_side_deadband = std::max(
    0.0,
...
public:
  ...
  : Node("mpc_controller"), config_path_(config_path), ref_vel_config_path_(ref_vel_path)
  {
    ...
    adaptive_reverse_retry_tracker_ =
      std::make_unique<stuck_recovery::AdaptiveReverseRetryTracker>(
      cfg_.stuck_recovery.adaptive_reverse_retry);
    ...
    RCLCPP_INFO(
      ...
      cfg_.stuck_recovery.core.supervisor.rejoin_lateral_regression_margin_m,
    ...
    RCLCPP_INFO(
      ...
      cfg_.stuck_recovery.adaptive_reverse_retry.enabled ? "enabled" : "disabled",
      cfg_.stuck_recovery.adaptive_reverse_retry.multiplier,
      cfg_.stuck_recovery.adaptive_reverse_retry.maximum_target_distance_m,
    ...
    if (mpc_cfg_.v2x_behavior.overtake_line.enabled) {
      RCLCPP_INFO(
        ...
        mpc_cfg_.v2x_behavior.overtake_line.entry_retry_cooldown_sec,
      ...
      RCLCPP_INFO(
        ...
        "slow_target_max=%.2f m/s, unseparated_front_reserve=%.2f m",
      ...
      RCLCPP_INFO(
        ...
        mpc_cfg_.v2x_behavior.overtake_line.pass_horizon_absolute_time_limit,
        mpc_cfg_.v2x_behavior.overtake_line.pass_horizon_soft_distance_limit,
        mpc_cfg_.v2x_behavior.overtake_line.pass_horizon_absolute_distance_limit,
        ...
        mpc_cfg_.v2x_behavior.overtake_line.pass_horizon_hold_max_distance,
...
private:
    ...
    const char * reason, const V2XTrackingResetPolicy v2x_tracking_reset)
  {
    ...
    if (v2x_gap_planner_) {
      if (
        v2x_tracking_reset == V2XTrackingResetPolicy::PreserveHistory &&
        cfg_.stuck_recovery.core.enabled)
      {
        // Recovery must still observe a complete post-reset V2X epoch, while normal behavior
        // keeps the Ready-time position samples needed for the next velocity estimate.
        v2x_gap_planner_->begin_recovery_tracking_epoch();
      } else if (v2x_tracking_reset == V2XTrackingResetPolicy::ClearHistory) {
        v2x_gap_planner_->reset_all_tracking();
      }
    }
    ...
    if (adaptive_reverse_retry_tracker_) {
      adaptive_reverse_retry_tracker_->reset();
    }
  ...
  void awsim_state_callback(const String::SharedPtr msg)
  {
    ...
    if (recovery_state_changed) {
      ...
      if (normalized_state == "start") {
        reset_stuck_recovery_session(
          "AWSIM Start entered", V2XTrackingResetPolicy::PreserveHistory);
        race_started_ = true;
        ...
        if (
          race_started_ || normalized_state == "spawned" ||
          normalized_state == "grounded" || normalized_state == "finish")
        {
          reset_stuck_recovery_session(
            "AWSIM race inactive or session boundary",
            V2XTrackingResetPolicy::ClearHistory);
    ...
    const rclcpp::Time & control_time)
  {
      ...
      adaptive_reverse_retry_tracker_)
    {
      ...
      if (adaptive_reverse_retry_tracker_->observe_normal_forward_progress(
          std::max(0.0, actual_v) * control_period_sec))
      {
        RCLCPP_INFO(
          get_logger(),
    ...
    if (started_recovery_episode) {
      ...
      if (adaptive_retry) {
        RCLCPP_WARN(
          get_logger(),
          "Stuck recovery repeated after short rejoin: retry_level=%zu, "
          "fast_target=%.2f m, normal_target=%.2f m",
      ...
      recovery_last_output_->state_reason != output.state_reason)
    {
      RCLCPP_INFO(
        ...
        "coordinated=%d, reverse_only=%d, retry_force_reverse=%d, "
        "forward_probe=%d, static=%s, "
        ...
        "escape_direction=%s, escape_target=%.3f m, "
        ...
        adaptive_reverse_retry_tracker_ ?
        adaptive_reverse_retry_tracker_->retry_level() : 0U,
        ...
        stuck_recovery::to_string(escape_direction), escape_distance_target_m,
        input.recovery.recovery_escape_confirmed ? 1 : 0,
    ...
    const rclcpp::Time & control_time)
  {
    ...
    last_acc_ = 0.0;
    reset_stuck_recovery_session(
      "race-relaxed fault retry", V2XTrackingResetPolicy::PreserveHistory);

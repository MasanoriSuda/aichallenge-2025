...
bool source_valid(const shadow::Snapshot & source) noexcept
{
  ...
  const int expected_side_sign) noexcept
{
  ...
  expected_candidate.execution_side_sign = expected_side_sign;
  ...
  return
    ...
    actual.execution_side_sign == expected_side_sign &&
    actual.dynamic_obstacle_constraint_active &&
    actual.dynamic_obstacle_generation == expected.dynamic_obstacle_generation &&
    ...
    actual.applied_program_fingerprint == expected.applied_program_fingerprint &&
...
State Bank::state() const
{
  ...
  result.selected_execution_side_sign =
    snapshot_.selected_execution_side_sign;

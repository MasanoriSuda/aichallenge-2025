  ...
  const OvertakeKinematicRolloutRequest & request) noexcept
{
  ...
  if (
    ...
    std::isnan(request.target_lateral_velocity_decay_time_sec) ||
    request.target_lateral_velocity_decay_time_sec < 0.0 ||
    !std::isfinite(request.target_lateral_uncertainty_growth_mps) ||
    request.target_lateral_uncertainty_growth_mps < 0.0 ||
    !std::isfinite(request.target_lateral_uncertainty_max_m) ||
    request.target_lateral_uncertainty_max_m < 0.0 ||
    !std::isfinite(request.lateral_clearance_m) || request.lateral_clearance_m < 0.0 ||
    !std::isfinite(request.hard_longitudinal_distance_m) ||
    request.hard_longitudinal_distance_m < 0.0 ||
    !std::isfinite(request.deadline_margin_sec) || request.deadline_margin_sec < 0.0 ||
  ...
  const auto target_lateral_uncertainty_at = [&](const double time_sec) {
      return std::min(
        request.target_lateral_uncertainty_max_m,
        request.target_lateral_uncertainty_growth_mps * std::max(0.0, time_sec));
    };
  ...
  const double previous_physical_lateral_margin_m =
    std::abs(target_lateral_at(0.0) - mission_origin.lateral_target_m) -
    request.lateral_clearance_m;
  double previous_lateral_margin_m =
    previous_physical_lateral_margin_m - target_lateral_uncertainty_at(0.0);
  ...
  if (request.mission_path.shift_distance_m <= 1e-9) {
    ...
    resolution.minimum_pass_target_surface_clearance_m =
      previous_physical_lateral_margin_m;
    resolution.pass_target_uncertainty_checked = true;
    resolution.minimum_pass_target_uncertainty_adjusted_clearance_m =
  ...
  for (std::size_t step = 0U; step < maximum_steps; ++step) {
    ...
    const double physical_lateral_margin_m =
      std::abs(target_lateral_at(step_end_sec) - path.lateral_target_m) -
      request.lateral_clearance_m;
    const double applied_lateral_uncertainty_m =
      target_lateral_uncertainty_at(step_end_sec);
    const double lateral_margin_m =
      physical_lateral_margin_m - applied_lateral_uncertainty_m;
    resolution.maximum_applied_target_lateral_uncertainty_m = std::max(
      resolution.maximum_applied_target_lateral_uncertainty_m,
      applied_lateral_uncertainty_m);
    if (mission_distance_m + 1e-9 >= request.mission_path.shift_distance_m) {
          ...
          resolution.minimum_pass_target_uncertainty_adjusted_clearance_m))
      {
        ...
        resolution.minimum_pass_target_uncertainty_adjusted_clearance_m = std::min(
          resolution.minimum_pass_target_uncertainty_adjusted_clearance_m,
          lateral_margin_m);
      }
    }
...
{
  FrenetDpExecutionRefreshStitchResolution resolution;
  if (
    !std::isfinite(request.current_lateral_m) ||
    !std::isfinite(request.active_traveled_distance_m) ||
    request.active_traveled_distance_m < 0.0 ||
    !std::isfinite(request.preserved_prefix_distance_m) ||
  ...
  const bool active_path_valid = is_valid_frenet_dp_execution_path(
    request.active_path_distances_m, request.active_lateral_path_m);
  FrenetDpCorridorBranchResolution active_branch;
  active_branch.feasible = active_path_valid;
  ...
  for (std::size_t i = 0U; i < request.candidate_path_distances_m.size(); ++i) {
    ...
    if (active_path_valid) {
      ...
      if (active_query_m <= request.active_path_distances_m.back() + 1e-9) {
        const double sampled = sample_frenet_dp_corridor_path(
          active_branch, active_query_m);
        if (std::isfinite(sampled)) {
          anchor_lateral_m = sampled;
          resolution.used_active_path = true;
        }
      } else {
        anchor_lateral_m = request.active_lateral_path_m.back();
        resolution.used_active_path = true;
      }
    }
    ...
    if (distance_m <= request.preserved_prefix_distance_m + 1e-9) {
      anchor_weight = 1.0;
    } else if (distance_m < request.blend_end_distance_m - 1e-9) {
      const double ratio = std::clamp(
        (distance_m - request.preserved_prefix_distance_m) /
  ...
  const OvertakePassPlanRequest & request) noexcept
{
  ...
  if (
    ...
    mission.predicted_minimum_pass_target_uncertainty_adjusted_clearance_m < -1e-9)) ||
  ...
  const OvertakeMissionCandidateSelectionRequest & request) noexcept
{
  ...
  const auto numerically_valid = [&](const OvertakeMissionCandidate & candidate) {
      ...
      const bool pass_target_clearance_valid =
        ...
        candidate.predicted_minimum_pass_target_surface_clearance_m >=
        -kEpsilon);
      const bool pass_target_uncertainty_valid =
        !candidate.pass_target_uncertainty_checked ||
        (std::isfinite(
          candidate.predicted_minimum_pass_target_uncertainty_adjusted_clearance_m) &&
        candidate.predicted_minimum_pass_target_uncertainty_adjusted_clearance_m >=
        -kEpsilon &&
        std::isfinite(candidate.maximum_applied_pass_target_lateral_uncertainty_m) &&
...
const char * to_string(const ProgressiveEntryCompletionGateRejectReason reason) noexcept
{
  switch (reason) {
    case ProgressiveEntryCompletionGateRejectReason::None:
      return "none";
    case ProgressiveEntryCompletionGateRejectReason::Disabled:
      return "disabled";
    case ProgressiveEntryCompletionGateRejectReason::CompleteMission:
      return "complete-mission";
    case ProgressiveEntryCompletionGateRejectReason::InvalidInput:
      return "invalid-input";
    case ProgressiveEntryCompletionGateRejectReason::FrontDistanceReserve:
      return "front-distance-reserve";
    case ProgressiveEntryCompletionGateRejectReason::NoReturnTimeReserve:
      return "no-return-time-reserve";
  }
  return "unknown";
}
...
{
  ProgressiveEntryCompletionGateResolution resolution;
  const auto finite_non_negative = [](const double value) {
      return std::isfinite(value) && value >= 0.0;
    };
  if (
    !finite_non_negative(request.target_front_distance_m) ||
    !finite_non_negative(request.positive_closing_speed_mps) ||
    !finite_non_negative(request.no_return_front_distance_m) ||
    !finite_non_negative(request.minimum_unproven_front_distance_m) ||
    !finite_non_negative(request.minimum_no_return_time_sec))
  {
    return resolution;
  }
  ...
  resolution.admitted = true;
  if (!request.enabled) {
    resolution.reject_reason = ProgressiveEntryCompletionGateRejectReason::Disabled;
    return resolution;
  }
  if (!request.progressive_entry) {
    resolution.reject_reason = ProgressiveEntryCompletionGateRejectReason::CompleteMission;
    return resolution;
  }

  resolution.checked = true;
  const double distance_to_no_return_m = std::max(
    0.0, request.target_front_distance_m - request.no_return_front_distance_m);
  if (request.positive_closing_speed_mps > 1e-6) {
    resolution.time_to_no_return_sec =
      distance_to_no_return_m / request.positive_closing_speed_mps;
  }
  if (
    request.target_front_distance_m + 1e-9 <
    request.minimum_unproven_front_distance_m)
  {
    resolution.admitted = false;
    resolution.reject_reason =
      ProgressiveEntryCompletionGateRejectReason::FrontDistanceReserve;
    return resolution;
  }
  if (
    resolution.time_to_no_return_sec + 1e-9 <
  ...
  resolution.reject_reason = ProgressiveEntryCompletionGateRejectReason::None;
  return resolution;
}
...
const char * to_string(const MpccLiteShadowRejectReason reason) noexcept
{
  switch (reason) {
    ...
    case MpccLiteShadowRejectReason::BodyClearUnchecked:
      return "body_clear_unchecked";
    case MpccLiteShadowRejectReason::BodyClearInfeasible:
      return "body_clear_infeasible";
  ...
  const MpccLiteShadowMissionCandidateRequest & request) noexcept
{
  ...
  if (!std::isfinite(mission_target_clearance_m) || mission_target_clearance_m < -1e-9)
  {
  ...
  const MpccLiteRecedingPrefixCandidateRequest & request) noexcept
{
  ...
  if (
    !std::isfinite(request.terminal_time_sec) || request.terminal_time_sec < 0.0 ||
    !std::isfinite(request.terminal_distance_m) || request.terminal_distance_m < 0.0)
  {
    return reject(MpccLiteShadowRejectReason::InvalidCandidate);
  }
  ...
  candidate.available = true;
  candidate.rear_clear_required = true;
  candidate.rear_clear_feasible = mission.body_clear_deadline_feasible;
  candidate.predicted_rear_clear_time_sec =
    mission.predicted_body_clear_time_sec + request.terminal_time_sec;
  candidate.predicted_rear_clear_distance_m =
    mission.predicted_body_clear_distance_m + request.terminal_distance_m;
  candidate.predicted_minimum_speed_mps = mission.predicted_minimum_ego_speed_mps;
  const auto finite_reserve = [](const double metric, const double fallback) {
      return std::isfinite(metric) ? std::max(0.0, metric) : std::max(0.0, fallback);
    };
  ...
  if (!mission.body_clear_deadline_feasible) {
    return reject(MpccLiteShadowRejectReason::BodyClearInfeasible);
  }
  if (
    !mission.pass_target_uncertainty_checked &&
    !mission.pass_target_clearance_checked)
  ...
  const bool numeric_valid =
    std::isfinite(candidate.predicted_rear_clear_time_sec) &&
    candidate.predicted_rear_clear_time_sec >= 0.0 &&
    std::isfinite(candidate.predicted_rear_clear_distance_m) &&
    candidate.predicted_rear_clear_distance_m >= 0.0 &&
    std::isfinite(candidate.predicted_minimum_speed_mps) &&
    candidate.predicted_minimum_speed_mps >= 0.0 &&
    std::isfinite(candidate.maximum_lateral_accel_mps2) &&
    candidate.maximum_lateral_accel_mps2 >= 0.0 &&
    std::isfinite(candidate.lateral_motion_m) && candidate.lateral_motion_m >= 0.0;
  if (!numeric_valid) {
    return reject(MpccLiteShadowRejectReason::InvalidCandidate);
  }
  candidate.hard_feasible = true;
...
{
  MpccLitePrefixExecutionResolution resolution;
  const auto reject = [&](const MpccLitePrefixExecutionRejectReason reason) {
      resolution.valid = true;
  ...
  if (!request.candidate_progressive) {
    return reject(MpccLitePrefixExecutionRejectReason::NotProgressive);
  }
  if (!request.candidate_feasible) {
  ...
  if (!request.body_clear_deadline_feasible) {
    return reject(MpccLitePrefixExecutionRejectReason::BodyClearInfeasible);
  }
  if (!request.target_clearance_checked) {
    return reject(MpccLitePrefixExecutionRejectReason::TargetClearanceUnchecked);
  }
  if (
    !finite_non_negative(request.minimum_target_surface_clearance_m))
  {
    return reject(MpccLitePrefixExecutionRejectReason::TargetClearanceInfeasible);
  }
  if (
  ...
  if (
    ...
    request.remaining_time_budget_sec + 1e-9)
  {
    resolution.reason = MpccLitePrefixExecutionRejectReason::TimeBudgetExceeded;
    return resolution;
  }
  if (
...
const char * to_string(const MpccLitePrefixExecutionRejectReason reason) noexcept
{
  switch (reason) {
    case MpccLitePrefixExecutionRejectReason::None:
      return "none";
    case MpccLitePrefixExecutionRejectReason::Inactive:
      return "inactive";
    case MpccLitePrefixExecutionRejectReason::NoReturn:
      return "no-return";
    case MpccLitePrefixExecutionRejectReason::SafeSeparation:
      return "safe-separation";
    case MpccLitePrefixExecutionRejectReason::NotProgressive:
      return "not-progressive";
    case MpccLitePrefixExecutionRejectReason::CandidateInfeasible:
      return "candidate-infeasible";
    case MpccLitePrefixExecutionRejectReason::BodyClearUnchecked:
      return "body-clear-unchecked";
    case MpccLitePrefixExecutionRejectReason::BodyClearInfeasible:
      return "body-clear-infeasible";
    case MpccLitePrefixExecutionRejectReason::TargetClearanceUnchecked:
      return "target-clearance-unchecked";
    case MpccLitePrefixExecutionRejectReason::TargetClearanceInfeasible:
      return "target-clearance-infeasible";
    case MpccLitePrefixExecutionRejectReason::InvalidPrediction:
      return "invalid-prediction";
    case MpccLitePrefixExecutionRejectReason::WallReserveInsufficient:
      return "wall-reserve-insufficient";
    case MpccLitePrefixExecutionRejectReason::TimeBudgetExceeded:
      return "time-budget-exceeded";
    case MpccLitePrefixExecutionRejectReason::DistanceBudgetExceeded:
...
const char * to_string(const MpccLiteAuthorityAction action) noexcept
{
  switch (action) {
    case MpccLiteAuthorityAction::None:
      return "none";
    case MpccLiteAuthorityAction::SelectEntry:
      return "entry";
    case MpccLiteAuthorityAction::KeepCurrent:
      return "hold";
    case MpccLiteAuthorityAction::BeginReturn:
      return "return";
    case MpccLiteAuthorityAction::ReplaceActive:
      return "replace";
  }
  return "unknown";
}
...
{
  MpccLiteAuthorityResolution resolution;
  const auto branch_side = [](const MpccLiteShadowBranch branch) {
      return branch == MpccLiteShadowBranch::Left ? 1 :
             branch == MpccLiteShadowBranch::Right ? -1 : 0;
    };
  const bool active_side_valid =
    request.active_side_sign == -1 || request.active_side_sign == 0 ||
    request.active_side_sign == 1;
  if (!active_side_valid) {
    return resolution;
  }
  resolution.valid = true;
  if (
    !request.enabled || !request.resolution_valid || !request.resolution_found ||
    request.runtime_hard_fault)
  {
    return resolution;
  }
  ...
  if (!request.active_mission) {
    return resolution;
  }
  if (
    request.selected_branch == MpccLiteShadowBranch::CurrentSideHold &&
    request.active_side_sign != 0)
  {
    resolution.action = MpccLiteAuthorityAction::KeepCurrent;
    resolution.selected_side_sign = request.active_side_sign;
    return resolution;
  }
  if (
    request.selected_branch == MpccLiteShadowBranch::Return &&
    (request.return_active || request.return_admitted))
  {
    resolution.action = MpccLiteAuthorityAction::BeginReturn;
    resolution.selected_side_sign = request.active_side_sign;
    return resolution;
  }
  if (
...
{
  DynamicObstacleLateralEscapeQualificationHoldResolution resolution;
  if (!request.candidate_active) {
    resolution.reason =
      DynamicObstacleLateralEscapeQualificationHoldReason::NotCandidate;
    return resolution;
  }
  if (request.tracking_solution_qualified) {
    resolution.reason =
      DynamicObstacleLateralEscapeQualificationHoldReason::AlreadyQualified;
    return resolution;
  }
  if (!request.previous_control_available) {
    resolution.reason =
  ...
  if (
    ...
    !std::isfinite(request.previous_command_speed_mps) ||
    !std::isfinite(request.previous_command_steering_rad) ||
    !std::isfinite(request.maximum_steering_rad) ||
    request.current_speed_mps < 0.0 ||
    request.previous_command_speed_mps < 0.0 ||
    request.maximum_steering_rad < 0.0)
  {
    resolution.reason =
  ...
  resolution.hold = true;
  // Do not turn a rejected candidate into either acceleration or braking.
  // The next 40 Hz cycle is already target/side-backoff scoped and rebuilds
  // ordinary Follow; this command only bridges that one-cycle handoff.
  resolution.speed_mps = std::min(
    request.current_speed_mps, request.previous_command_speed_mps);
  resolution.steering_rad = std::clamp(
    request.previous_command_steering_rad,
    -request.maximum_steering_rad, request.maximum_steering_rad);
  resolution.reason =
...
const char * to_string(
  const DynamicObstacleLateralEscapeQualificationHoldReason reason) noexcept
{
  switch (reason) {
    case DynamicObstacleLateralEscapeQualificationHoldReason::NotCandidate:
      return "not-candidate";
    case DynamicObstacleLateralEscapeQualificationHoldReason::AlreadyQualified:
      return "already-qualified";
    case DynamicObstacleLateralEscapeQualificationHoldReason::PreviousControlUnavailable:
      return "previous-control-unavailable";
    case DynamicObstacleLateralEscapeQualificationHoldReason::InvalidInput:
      return "invalid-input";
    case DynamicObstacleLateralEscapeQualificationHoldReason::HoldLastFeasible:
      return "hold-last-feasible";
  }
  return "unknown";
}
...
{
  if (request.low_speed_behavior_active) {
    return true;
  }

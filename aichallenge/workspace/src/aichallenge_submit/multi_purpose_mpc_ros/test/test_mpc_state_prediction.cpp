...
TEST(MpcStatePrediction, MotionObservationRejectsInvalidSourceAndTargetEpochs) {
  using p::MotionObservation;
  const MotionObservation observation{10., {1.,2.,.3},2.,.15};
  EXPECT_FALSE(p::predict_constant_twist_observation(observation,9.99,.5));
  EXPECT_FALSE(p::predict_constant_twist_observation(observation,10.51,.5));
  EXPECT_FALSE(p::predict_constant_twist_observation(observation,10.,0.));
  auto invalid=observation;invalid.stamp_sec=std::numeric_limits<double>::quiet_NaN();
  EXPECT_FALSE(p::predict_constant_twist_observation(invalid,10.,.5));
  invalid=observation;invalid.yaw_rate_radps=std::numeric_limits<double>::quiet_NaN();
  EXPECT_FALSE(p::predict_constant_twist_observation(invalid,10.,.5));
  EXPECT_FALSE(p::predict_constant_twist_observation(observation,std::numeric_limits<double>::infinity(),.5));
}

...
TEST(MpccArchitectureSnapshot, NativeInitializerIsSerializedAndBoundToFingerprint)
{
  for (const auto policy : {
      mpcc_rate_resolved_adapter::InitialTangentPolicy::ReferenceSteeringWithRestLaunch,
      mpcc_rate_resolved_adapter::InitialTangentPolicy::SteeringBeforeDrive}) {
    const auto root = output_root("native-initializer-roundtrip-" + std::to_string(static_cast<int>(policy)));
    std::filesystem::remove_all(root);
    auto snapshot = make_interaction_snapshot(mpcc_execution_contract::ControlIntent::Pass);
    const auto original_fingerprint = fingerprint_interaction_snapshot(snapshot);
    snapshot.request.initial_tangent_policy = policy;
    ...
    const auto written = record_failure(snapshot,make_assembly_request(),make_valid_problem(),std::nullopt,
      persistent_osqp::SolveOutcome{},PipelineStage::Initial,"native-initializer-" + std::to_string(static_cast<int>(policy)),"intentional test",root);
    ASSERT_EQ(written.status, RecordStatus::Written) << written.detail;

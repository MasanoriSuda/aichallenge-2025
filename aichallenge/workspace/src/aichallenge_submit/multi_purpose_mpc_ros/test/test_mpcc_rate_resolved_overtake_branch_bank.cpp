...
shadow::Snapshot source_snapshot(
  const std::uint64_t sequence, const int selected_side_sign = 1,
  const contract::ControlIntent intent = contract::ControlIntent::Pass)
{
  shadow::Snapshot source;
  source.identity = execution::Identity{
    sequence, source_context(sequence, selected_side_sign, intent),
  ...
  const contract::ControlIntent intent = contract::ControlIntent::Pass)
{
  ...
  value.identity = execution::Identity{
    sequence, source_context(sequence, side_sign, intent),
    10.0 + static_cast<double>(sequence)};
  ...
  const contract::ControlIntent intent = contract::ControlIntent::Pass)
{
  auto execution_artifact =
    ...
    artifact(sequence, side_sign, intent));
  const auto physical_source = physical_snapshot(execution_artifact->identity);
  physical::Result result;
  result.identity = physical_source.identity;
  result.outcome = physical::Outcome::Accepted;
  result.diagnostic.reason = contract::PhysicalWallCertificateReason::Accepted;
  result.completed_sec = execution_artifact->identity.snapshot_sec + 0.02;
  result.compute_ms = 20.0;
  result.detail = "accepted";
  return certified::build(execution_artifact, physical_source, result).plan;
}

TEST(OvertakeBranchBank, PublishesBothSidesFromOnePassEpoch)
{
  bank::Bank subject;
...
TEST(OvertakeBranchBank, AcceptsShiftOutAndAtomicallyInvalidatesOlderSides)
{
  ...
  ASSERT_EQ(
    subject.replace(
      source_snapshot(5U, -1, contract::ControlIntent::ShiftOut),
      plan(5U, -1, contract::ControlIntent::ShiftOut),
      plan(5U, 1, contract::ControlIntent::ShiftOut)),
    bank::ReplaceReason::Accepted);

  EXPECT_EQ(
    subject.replace(source_snapshot(6U, -1), nullptr, nullptr),
    bank::ReplaceReason::Accepted);
  const auto stored = subject.snapshot();
...
TEST(OvertakeBranchBank, RejectsWrongEpochOrWrongSide)
{
  bank::Bank subject;
  EXPECT_EQ(
    subject.replace(source_snapshot(5U, 1), plan(4U, -1), plan(5U, 1)),
    bank::ReplaceReason::InvalidNegativePlan);
  EXPECT_EQ(
    subject.replace(source_snapshot(5U, 1), plan(5U, 1), nullptr),
    bank::ReplaceReason::InvalidNegativePlan);
  EXPECT_EQ(subject.state().latest_source_sequence, 0U);
}

TEST(OvertakeBranchBank, RejectsNormalAndReturnSources)
{
  bank::Bank subject;
  EXPECT_EQ(
    subject.replace(
      source_snapshot(5U, 1, contract::ControlIntent::Cruise),
      nullptr, nullptr),
    bank::ReplaceReason::InvalidSource);
  EXPECT_EQ(
    subject.replace(
      source_snapshot(6U, 1, contract::ControlIntent::Return),
      nullptr, nullptr),
    bank::ReplaceReason::InvalidSource);
}

TEST(OvertakeBranchBank, RejectsStaleSourceWithoutChangingNewerEpoch)
{
  bank::Bank subject;
  const auto newer_negative = plan(6U, -1);
  ASSERT_EQ(
    subject.replace(source_snapshot(6U, 1), newer_negative, plan(6U, 1)),
    bank::ReplaceReason::Accepted);
  EXPECT_EQ(
    subject.replace(source_snapshot(5U, 1), plan(5U, -1), plan(5U, 1)),
    bank::ReplaceReason::StaleSource);
  EXPECT_EQ(subject.snapshot().negative_plan, newer_negative);
}

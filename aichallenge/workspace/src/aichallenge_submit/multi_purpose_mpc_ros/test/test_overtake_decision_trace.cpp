...
TEST(OvertakeDecisionTrace, DistinguishesQualificationFromActiveAuthority) {
  trace::DecisionTrace decision;
  decision.target_id = "d2";
  decision.requested = true;
  decision.primary = ready_candidate(0, -1);
  ...
  EXPECT_EQ(
    trace::classify_outcome(decision),
    trace::DecisionOutcome::QualificationPendingPrimary);
  EXPECT_NE(
    trace::format_decision_trace(decision).find(
      "outcome=qualification-pending-primary"),
    std::string::npos);
  ...
  EXPECT_EQ(
    trace::classify_outcome(decision),
    trace::DecisionOutcome::QualificationPendingAlternate);

  decision.tracking_qualified = true;
  EXPECT_EQ(
    trace::classify_outcome(decision),
    trace::DecisionOutcome::ActiveAlternate);
}

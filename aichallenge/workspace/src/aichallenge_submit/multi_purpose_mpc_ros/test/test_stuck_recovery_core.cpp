...
TEST(RecoverySupervisor, RearClearanceCanRecoverWhileWaiting)
{
  ...
  input.now_sec = now;
  input.rear_v2x_clear = true;
  EXPECT_EQ(supervisor.update(input).reason, RecoveryReason::ClearanceCheck);
  EXPECT_EQ(supervisor.state(), RecoveryState::CheckClearance);
  now += 0.01;
  input.now_sec = now;
...
TEST(StuckRecoveryCore, EnumStringsAreStableAndUnknownSafe)
{
  ...
  EXPECT_STREQ(to_string(RecoveryReason::RearInformationIncomplete),
    "rear_information_incomplete");
  EXPECT_STREQ(to_string(ExecutionMode::SimulationOnlyBlocked), "simulation_only_blocked");

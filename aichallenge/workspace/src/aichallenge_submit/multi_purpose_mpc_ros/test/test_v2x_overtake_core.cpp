...
TEST(V2XOvertakeCoreSpeed, LatchedPassRetainsCurrentWorldPrefixAcrossFutureConflict)
{
  ...
  request.forward_progress_extension_enabled = true;
  request.pass_phase = true;
  request.latched_pass_clearance_acquired = true;
  request.fresh_forward_progress = false;
  request.mission_elapsed_sec = 4.0;
  request.mission_traveled_m = 18.0;
  request.absolute_maximum_sec = 10.0;
  ...
  request.mission_elapsed_sec = request.absolute_maximum_sec;
  EXPECT_FALSE(target_bound_execution_hold_budget_available(request));
  request.mission_elapsed_sec = 4.0;
  request.latched_pass_clearance_acquired = false;
  EXPECT_FALSE(target_bound_execution_hold_budget_available(request));
}
...
{
  DynamicObstacleLateralEscapeAuthorityRequest request;
  request.enabled = true;
  request.dynamic_obstacle_target_active = true;
  request.follow_state_active = true;
  request.gap_planner_active = true;
  request.gap_planner_feasible = true;
  request.gap_planner_owns_lateral_bounds = true;
  request.pass_side_sign = 1;
...
TEST(V2XOvertakeCoreLowSpeedBypass, FeasibleDynamicLateralEscapeSuppressesFollowCap)
{
  const auto resolution = resolve_dynamic_obstacle_lateral_escape_authority(
    dynamic_lateral_escape_request());
  ...
  EXPECT_EQ(resolution.reason, DynamicObstacleLateralEscapeAuthorityReason::Accepted);
}
...
{
  auto request = dynamic_lateral_escape_request();
  request.gap_planner_feasible = false;
  auto resolution = resolve_dynamic_obstacle_lateral_escape_authority(request);
  EXPECT_FALSE(resolution.active);
  EXPECT_EQ(
    resolution.reason,
    DynamicObstacleLateralEscapeAuthorityReason::PlannerInfeasible);

  request = dynamic_lateral_escape_request();
  request.gap_planner_owns_lateral_bounds = false;
...
TEST(V2XOvertakeCoreLowSpeedBypass, DynamicLateralEscapeExplainsEveryAdmissionGate)
{
  ...
  request.enabled = false;
  EXPECT_EQ(
    resolve_dynamic_obstacle_lateral_escape_authority(request).reason,
    DynamicObstacleLateralEscapeAuthorityReason::Disabled);

  request = dynamic_lateral_escape_request();
  request.dynamic_obstacle_target_active = false;
  EXPECT_EQ(
    resolve_dynamic_obstacle_lateral_escape_authority(request).reason,
    DynamicObstacleLateralEscapeAuthorityReason::TargetInactive);

  request = dynamic_lateral_escape_request();
  request.gap_planner_active = false;
  EXPECT_EQ(
    resolve_dynamic_obstacle_lateral_escape_authority(request).reason,
    DynamicObstacleLateralEscapeAuthorityReason::PlannerInactive);

  request = dynamic_lateral_escape_request();
  request.emergency_brake_active = true;
  EXPECT_EQ(
    resolve_dynamic_obstacle_lateral_escape_authority(request).reason,
    DynamicObstacleLateralEscapeAuthorityReason::EmergencyBrake);

  request = dynamic_lateral_escape_request();
  request.solver_recovery_active = true;
  EXPECT_EQ(
    resolve_dynamic_obstacle_lateral_escape_authority(request).reason,
    DynamicObstacleLateralEscapeAuthorityReason::SolverRecovery);

  request = dynamic_lateral_escape_request();
  request.pass_side_sign = 0;
  EXPECT_EQ(
    resolve_dynamic_obstacle_lateral_escape_authority(request).reason,
    DynamicObstacleLateralEscapeAuthorityReason::InvalidSide);
}
...
TEST(V2XOvertakeCoreLowSpeedBypass, DynamicLateralEscapeRejectsTinyOrWrongSideShift)
{
  auto request = dynamic_lateral_escape_request();
...
TEST(V2XOvertakeCoreMpccLite, GateATacticalInputUsesCurrentWorldMissionWhenIdle)
{
  ...
  const auto resolution = resolve_rate_resolved_gate_a_tactical_input(
    RateResolvedGateATacticalInputRequest{
      false, 0, -1, current_world, std::nullopt, runtime, 0, std::nullopt});
  ASSERT_TRUE(resolution.valid);
...
TEST(V2XOvertakeCoreMpccLite, GateATacticalInputUsesAsyncHomotopyWhenLiveMissionIsAbsent)
{
  using multi_purpose_mpc_ros::v2x_overtake_core::OvertakeMissionCandidate;
  using multi_purpose_mpc_ros::v2x_overtake_core::
    RateResolvedGateATacticalInputRequest;
  ...
  OvertakeMissionCandidate async_hint;
  async_hint.feasible = true;
  async_hint.pass_side_sign = 1;

  const auto resolution = resolve_rate_resolved_gate_a_tactical_input(
    RateResolvedGateATacticalInputRequest{
      false, 0, 0, std::nullopt, std::nullopt, std::nullopt, 1, async_hint});
...
TEST(V2XOvertakeCoreMpccLite, GateATacticalInputPrefersCurrentWorldOverAsyncHomotopy)
{
  using multi_purpose_mpc_ros::v2x_overtake_core::OvertakeMissionCandidate;
  using multi_purpose_mpc_ros::v2x_overtake_core::
    RateResolvedGateATacticalInputRequest;
  ...
  OvertakeMissionCandidate live;
  live.feasible = true;
  live.pass_side_sign = -1;
  OvertakeMissionCandidate async_hint = live;
  async_hint.pass_side_sign = 1;

  const auto resolution = resolve_rate_resolved_gate_a_tactical_input(
    RateResolvedGateATacticalInputRequest{
      false, 0, -1, live, std::nullopt, std::nullopt, 1, async_hint});
...
TEST(V2XOvertakeCoreMpccLite, GateATacticalInputNeverUsesAsyncEntryWhenActive)
{
  using multi_purpose_mpc_ros::v2x_overtake_core::OvertakeMissionCandidate;
  using multi_purpose_mpc_ros::v2x_overtake_core::
    RateResolvedGateATacticalInputRequest;
  ...
  const auto resolution = resolve_rate_resolved_gate_a_tactical_input(
    RateResolvedGateATacticalInputRequest{
      true, -1, 0, std::nullopt, std::nullopt, std::nullopt, 1, async_hint});
...
TEST(V2XOvertakeCoreMpccLite, GateATacticalInputMirrorsActiveExecutionOrder)
{
  ...
  const auto resolution = resolve_rate_resolved_gate_a_tactical_input(
    RateResolvedGateATacticalInputRequest{
      true, -1, 1, cross_side, same_side, cross_side, 0, std::nullopt});
  ASSERT_TRUE(resolution.valid);
  ASSERT_TRUE(resolution.mission.has_value());
  EXPECT_EQ(resolution.selected_side_sign, -1);
  EXPECT_EQ(
...
TEST(V2XOvertakeCoreMpccLite, GateATacticalInputCertifiesActiveCrossSideRequest)
{
  ...
  const auto resolution = resolve_rate_resolved_gate_a_tactical_input(
    RateResolvedGateATacticalInputRequest{
      true, -1, -1, old_current_world, std::nullopt, cross_side, 0, std::nullopt});
  ASSERT_TRUE(resolution.valid);
  ASSERT_TRUE(resolution.mission.has_value());
  EXPECT_EQ(resolution.selected_side_sign, 1);
  EXPECT_EQ(
...
TEST(V2XOvertakeCoreMpccLite, GateATacticalInputNeverUsesNewEntryMissionWhenActive)
{
  ...
  const auto resolution = resolve_rate_resolved_gate_a_tactical_input(
    RateResolvedGateATacticalInputRequest{
      true, -1, -1, current_world, std::nullopt, std::nullopt, 0, std::nullopt});
  EXPECT_FALSE(resolution.valid);
  EXPECT_FALSE(resolution.mission.has_value());
...
TEST(V2XOvertakeCoreMpccLite, GateATacticalInputRejectsInvalidCurrentWorldMission)
{
  ...
  const auto resolution = resolve_rate_resolved_gate_a_tactical_input(
    RateResolvedGateATacticalInputRequest{
      false, 0, 1, invalid, std::nullopt, std::nullopt, 0, std::nullopt});

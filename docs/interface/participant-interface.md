...
2026 の障害物停止、追走、追い越し判断では `/v2x/vehicle_positions` のみを正入力として扱う。Gate2 の `use_v2x_overtake` も同じ topic を使い、追加の ROS topic / service 契約は増やさない。`/scan`、`/image_raw`、CSV 障害物、`/aichallenge/objects` は公式障害物入力ではない。

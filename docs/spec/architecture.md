...
ローカルAWSIMは同時刻のIMU絶対姿勢を校正済みTFで変換してGNSSレバーアームを戻し、
その観測姿勢で初期化する。実車の既定は明示されたraceline初期化を維持する。
`/set_initial_pose`の責務とGNSS共分散は維持する。[自己位置入力の校正](localization-calibration.md)を参照。

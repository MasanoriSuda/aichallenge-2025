...
[設計・検証](../../.steering/20260910-mpcc-empirical-plant/receiver-input-enclosure/rejoin-full-wall-feedback-design.md)。
...
[監査・証拠](../../.steering/20260910-mpcc-empirical-plant/receiver-input-enclosure/recovery-steering-live-audit.md)。

## Recovery後退候補と補助誘導の分離（2026-09-13）
...
[設計](../../.steering/20260910-mpcc-empirical-plant/receiver-input-enclosure/recovery-steering-population-design.md)、[検証](../../.steering/20260910-mpcc-empirical-plant/receiver-input-enclosure/recovery-steering-population-validation.json)。
...
[設計と検証](../../.steering/20260910-mpcc-empirical-plant/receiver-input-enclosure/reserved-program-order-design.md)。

## 単車検証と復帰後の予約計算（2026-09-13）
...
[監査と証拠](../../.steering/20260910-mpcc-empirical-plant/receiver-input-enclosure/reserved-program-order-audit.md)。
...
Rejoinの診断適用漏れはR719旧失敗→R720全50/source118、build133/package120で修正。
R721は実ビルドで元の壁拒否を保持する。
...
Stop用の地図範囲は通常QPのprogress上限から独立して確保する。新モデルの
公称停止ロールアウトを地図データ確保の目安にし、現在の速度上限・1周期の
最大wire入力・その後の制動を使う。これは停止距離の保証値ではなく、実際の
旋回Stopには同じ非線形・壁・他車認証を要求する。追加点は既存地図から取り、
...
早期ShiftOut side replanの適用判定も同じpublication identityを認識する。固定Missionのgeometryを
破棄しても未確定状態へ戻さず、公開済みstateless encounterに旧来の相対左右順序だけを根拠とする
side反転・Recovery要求を出さない。壁・他車・target連続性と毎周期の証明は独立して維持する。
詳細は[再選択の所有状態修正](../../.steering/20260910-mpcc-empirical-plant/side-replan-ownership-design.md)を参照。
...
- 新規Overtakeのpass sideは、内側が最低連続距離を満たしただけでは確定しない。`v2x_overtake_line_side_quality_selection_enabled=true`では左右それぞれのinflated vehicle side clearance、gap corridor width、連続open距離、ShiftOut preflight必要横加速度から品質値を作り、`v2x_overtake_line_side_quality_min_score_advantage`以上の差があれば高品質側を選ぶ。差が小さい場合だけ従来の幾何preferred sideをtie breakに使う。commit済みShiftOut/Passでもno-return前かつ現在車体が非重複で予測が有効なら左右のfull Missionを再評価でき、現在側の予測sweep重複は再評価禁止ではなく現在Mission不成立として扱う。反対側のShiftOut・Pass・Return全体が成立し、debounceを満たした場合だけfrozen Missionを一度だけatomicに置換する。予測不能、現在車体重複、no-return後、置換回数上限後はsideを変えない。別系統のearly side replanは固定Missionもpublisher-boundなstateless sourceも持たないShiftOutに限る。その適用範囲でlocked targetが`v2x_overtake_line_side_replan_target_guard_distance`以内で選択側へegoを越えたcourse-relative orderingを`v2x_overtake_line_early_side_replan_stable_sec`継続した場合、左右を再評価する。横移動量が`v2x_overtake_line_early_side_replan_max_lateral_progress`以下かつ前進距離が`v2x_overtake_line_early_side_replan_max_traveled_distance`以下なら、成立した反対側へ現在位置から一度だけShiftOutを再計画する。window外、再計画済み、または反対側不成立なら直接side反転せずRecoveryへ移し、旧sideをentry retry blockする。`output/20260728-000047/d1`では左右成立から内側を選択し、target相対横位置が`-0.93 m`から`+0.85 m`へ変化して左右が再び成立してもfixed sideを保持し、最後は`wall=front`と8回連続solver failureになったため導入した。debugの`left_q`、`right_q`、`side_conflict`、`replan_window`、`replan_candidate`、`replan_stable`、`replan_ready`、`replan_abort`で判断を追跡する。過去A/Bで不安定だった生の`v2x_prediction_use_course_lateral_velocity`は今回有効化せず、現在orderingの継続時間で観測チャタリングを抑える。これは2025 AWSIM `make dev2`向けの暫定挙動である。
...
これは保存場面の原因比較であり、この比較だけでは予測producerの統合受入れを示さない。
...
### 2026-09-09 公開済み縦指令と観測応答を使う遅延予測

予測入力は最終publisherが実際に公開した加速度と、その既存のcontrol originである。
新しく求解した未公開指令を使わない。指令加速度だけへの置換は、単車の移動中速度予測の
平均誤差を0.0558から0.1655m/sへ悪化させたため不採用とした。指令と実際の加減速には
走行抵抗などの差があるため、次の同時刻の応答残差を保持する。

`response_residual = LP(odometry速度差 / source時刻差) - LP(同じ区間の公開済み指令積分 / source時刻差)`

二つのLPは同じodometry callbackで更新し、既存twist2accelと同じ0.9のgainを使う。
予測区間を公開済み指令の切替時刻で分割し、各区間を`指令加速度 + response_residual`で
積分する。位置・速度・yaw応答・壁検査用の全経路はこの一つのrolloutから得る。
MPCC内の独立したacceleration購読と、観測加速度を区間全体へ保持する経路は削除した。
他の利用者向けの`/localization/acceleration`は維持する。gainは設定項目
`longitudinal_response_filter_gain`であり、この失敗場面への数値調整はしていない。

同一source時刻の再受信で微分時間やfilter更新を作らない。時刻逆行・不正値ではobserverを
初期化し、既存`odom_timeout_sec`を超えた欠損後は新しい正時間差の観測を待つ。
指令履歴は同じ有効観測区間の直前の入力と、その後の切替だけを保持する。
受信済みodometryが独立配信の`/clock`キャッシュより新しい場合は、元の整数stampを
decision・予測・証明・最終公開の共通時刻にする。時計逆行と既存観測期限を超えるずれは
拒否する。observer側の未来時刻拒否を緩めず、時刻を所有するproducerを修正する。

保存済みStop453/world993の再証明は予測速度1.420476m/sで停止末端を通過した。
時計修正後の25package buildと60CTestgroup／2359記録は合格したが、統合受入れは別に
検証する。0.13秒は既存の予測モデルであり、AWSIM縦入力の正確な適用位相は未同定。
...
2026-09-10追記: 動力学比較で使ったUnity Rigidbody原点と制御の`base_link`は別で、
`base_link`は車体原点から後方0.485m・上方0.05mにある。COMは`base_link`から
前方約0.175992m。原点を統一した比較でも動的モデルは改善するが、1秒先の
最大位置誤差約0.676mが残り、採用条件は満たしていない。姿勢状態の追加が必須と
旧比較だけで断定しない。修正済みGNSSの水平位置は、物理姿勢から変換した
`base_link`と4.013mm以内で一致した。
受信後操舵・駆動・接地条件付き静止のnative再生は20147tickで一致するが、
未来の接地・指令適用時刻を制御側から得られることを意味しない。
[修正した比較と棄却結果](../../.steering/20260910-mpcc-shared-plant-model/results.md)を参照。

接地点の向きは前物理ステップの車体姿勢・操舵に対応するが、この時刻差をモデルへ
反映するだけでは1秒先の最大位置誤差約0.693mが残る。DDS遅延設定へ混ぜない。
...
#### 完全停止の終端時間と可行性候補（2026-09-10、2025由来の暫定）

現行の完全停止ワーカーは、共有車体モデルの直線制動で求める公称停止時間と、
元の問題が持つ最大時間範囲の2候補を扱う。同じ時間になる場合は1候補にする。
前者は停止時間の保証ではなく、全9状態の自由入力最適化を始める候補時間である。
後者は後方車の通過などで停止を遅らせる候補のために維持する。
各候補は時間刻みを明示して再封印し、全車の予測を同じ時刻へ再計算する。
停止候補の目的関数は可行性のみとし、走行用の進捗・速度報酬を持ち込まない。
`cost_schema_id`の`/zero-cost-rest-feasibility-v1`で以前の目的と区別する。

既存OSQPとSQPの反復上限、物理入力・壁・他車の制約、完全な車体静止の検査は維持する。
最初に全検査を通った候補を返し、候補間でも観測更新によるキャンセルを確認する。
実ワーカーと`--current-world-complete-rest-only`は同じ候補生成を使う。
短い候補の検査は実際に静止するまでを対象とし、5秒間や無期限の静止占有を保証しない。
...
最初の速度・操舵・加速度の完全一致を要求してから公開する。元の長いStop軌道を
そのまま実行済みとして記録して、短い認証済み制動列を破棄してはならない。

停止に使う横参照は、解が持つ横位置の列に、末端の横位置を保つコース上の区間を
明示的に接続する。接続区間は同じimmutable Stop地図の終端までに限る。通常の解の
horizonを少し越えただけで停止直前に参照が欠落しないようにするための生成規則であり、
追加区間がsolverの解や安全な軌道であるとは扱わない。元の参照点・厳密な範囲検査は
維持し、生成した停止列全体に同じnativeモデル、完全停止、壁・他車・指令一致の証明を
要求する。[原因と比較](../../.steering/20260910-mpcc-empirical-plant/terminal-reference-design.md)を参照。

stateless feedbackで公開した場合はStopも公開Bundleの由来として保持する。
変更していない元軌道の実行記録とは区別する。公開Bundleを再証明したStopの役割は
維持し、新しい認証済みnormalは既存の採用経路で置換できる。追加の待機・再試行・
猶予規則は設けない。格納・再証明・指令一致が成立しなければ従来のEmergencyへ進む。

dev2-r3のD2decision954再構成では、元の5秒Stopに代わり認証された0.6秒の停止列を
格納すると現在状態での再証明が通る。実際の957拒否は保存・再現されており、修正後の
統合受入れは別途必要。[原因・比較・検証](../../.steering/20260910-mpcc-empirical-plant/first-dev2-r3-failure.md)を参照。

完全停止後もStopは時間付き入力列として表現する。`terminal_body_rest_required`の
artifactで、両端の前後・横・旋回速度が正確にゼロ、加速度指令が非正、仮想進捗速度が
ゼロの区間だけは走行距離の重複を許す。距離の減少や移動・加速の偽の静止は拒否する。
物理再生とcontinuationも既存の静止suffix表現を引き継ぎ、速度許容差はゼロとする。
現在状態の壁・他車・停止末端証明と公開時の指令一致は引き続き必要。

これにより、元の停止軌道が期限を迎えた時に、認証済みの現在静止区間を格納できる。
...
2026-09-12: 未送信の前段指令を持つ非Follow候補は、元の前段＋新規指令列をすべて
含む独立な状態・時刻集合の証明を持てる。新規指令の開始時刻から元の停止時刻までを
証明し、前段の遅延入力、実際の送信履歴・ID、現在の障害物、元25ms送信窓を維持する。
前段の送信が元の期限まで遅れる場合を、新規操舵値の生成時点で考慮する。変更値には
完全な再予測と新しい証明IDが必要。Followの従来の初回窓証明はこの範囲を扱わない。
全26build、2574tests、実ライブラリ再生で確認。実行時の事前予約・待機候補の採用は
次の実装であり、現時点では通常の新規計画は引き続き前段数0である。
[前段入力を含む証明](../../.steering/20260910-mpcc-empirical-plant/receiver-input-enclosure/composite-prior-proof-design.md)。

2026-09-12 実行時接続: 公開済みの証明付き指令列から、計算中に送る予定の2指令の
元時刻と全IDを事前固定する。計算済み候補は元の開始時刻まで保持し、実際の前段履歴が
予定と一致したときだけ従来の現在状態・世界・送信前後判定へ渡す。計算中/待機候補は
1件に制限する。予約可能な候補では既存の長い証明済み指令列を先に試し、その加速区間と
制動区間をintentへ正しく反映する。起動時の前段0候補は従来の生成順を維持する。
...
2026-09-13 force-r2/R550–R558: 走行前に固定した接地候補は新規観測の平均誤差を
改善したが、Ready972で現在の壁証明を拒否し、採用・走行受入れは未達。元の入力範囲内
でも実速度は予測を外れ、位置より30ms古い速度の保持と接地変動の両方が寄与する。
公開値の共通過去時刻への一律補間にも鮮度・旋回・停止境界の反例がある。元のraw値、

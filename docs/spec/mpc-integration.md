...
- AWSIM状態を購読できるシミュレーションでは、`/awsim/state`が`Start`になるまでV2X追い越し・停止車両回避・回避stall watchdogを開始しない。MPCノード初期化時の`START!`ログはレース開始ではないため、発進前の待機時間を回避stallとして消費しない。`Start`遷移時にV2X状態、pass line、cooldownを初期化してから判定を開始する。AWSIM状態追跡を使わないlaunchでは従来どおりV2X判定を常時有効とする。
...
- Prepared開始時に静止車として確認したfront target IDはsession内で1台だけlatchし、その車両の発進速度が `v2x_moving_front_speed_threshold` 以下の立ち上がり中も同じグリッド車として扱う。target IDが変わる、side contextから外れる、moving-front閾値を超える、または猶予が終了すれば通常判定へ戻す。`v2x_start_grid_breakout_enabled=true` では、latchした静止グリッド車に限り、egoとfrontの横差が`v2x_start_grid_breakout_side_deadband`を超える場合は既にずれている側を初期候補とし、deadband内でほぼ整列している場合は左右両側を候補にする。gap plannerが選択したsideは同じtargetのbreakout中にlatchし、ShiftOut中の相対横位置変化だけで再選択・Follow復帰しない。deadbandは初期side選択専用であり、`v2x_overtake_line_min_target_separation`のline到達判定とは分離する。既存gap plannerが車両inflation・壁境界・連続gapを満たすside corridorを返した場合、通常の5 m entry guardより先に`Overtake`へ入る。generic gap targetの横加速度評価はcorridor中央への移動量を使い、実行するbounded OvertakeLineと一致しないためbreakout時だけ対象外とする。残りの横移動中はentry speed以上を保持し、pass-side line到達後に通常速度を解放する。corridorが不成立なら `FrontRiskLevel::EmergencyBrake` を即時適用する。breakoutが迂回するのは初期グリッド車由来の追従arbitrationだけで、overtake禁止WP、odometry、非finite入力、solver、control disable、collision、壁制約などのfail-safeは対象外である。ROS clockが非finiteまたはStart epochより巻き戻った場合は猶予をExpiredとして抑制しない。
